...
**Answer:** Across all four registered/engine MCP tool modules (`src/tensor_grep/cli/mcp_server.py`, `src/tensor_grep/cli/mcp_symbol_tools.py`, `src/tensor_grep/cli/mcp_audit_tools.py`, `src/tensor_grep/cli/mcp_rewrite_tools.py`), an audited population was reduced to exactly 54 closed-world authorized sites:
1. **Eliminated across 4 modules:**
   - 34 Class A broad `except Exception` internal error arms routed to `_sanitized_tool_error` / constant sanitized payloads, with full traceback and path details sent exclusively to `stderr`.
   - All narrow exception arms (`SessionStaleError`, `FileNotFoundError`, `ValueError`, `ConfigurationError`, `OSError`, `PolicyCommandsNotAllowedError`, `PolicyValidationError`) sanitized to return safe client messages while logging full details to `stderr` (all tool-level narrow handlers explicitly bind `as exc` and call `_log_tool_exception`).
   ...
   - Native rewrite/diff/search subprocess and embedded engine failures sanitized to prevent leaking secret tokens or candidate paths.
   - Untrusted native `stderr` is classified internally by `_classify_native_rewrite_failure` and logged server-side rather than returned verbatim.
2. **Authorized Closed-World Remaining (54 sites):**
   ...
   - `mcp_rewrite_tools.py`: 2 sites (all `PathConfinementError`).
   - Total = 54 sites (100% `PathConfinementError`).
   - Exactly 0 unauthorized sites across all 4 modules.
...
**Answer:**
1. **Poison Exception Fixtures & Direct FastMCP Tests:** Inject poison exceptions containing secret paths and tokens across broad and narrow handlers, cwd resolution failure, external candidate path refusals, and rewrite engine / diff / index-search subprocesses. Direct `mcp.call_tool` tests verify that poison strings appear in `stderr` (logging control) but are 100% absent from wire JSON payloads and structured data.
2. **Closed-World Structural AST Ratchet:** `test_broad_mcp_handlers_never_echo_raw_str_exc_ast_ratchet` and `test_narrow_mcp_handlers_never_echo_raw_exception_formatting_ast_ratchet` scan `mcp_server.py`, `mcp_symbol_tools.py`, `mcp_audit_tools.py`, and `mcp_rewrite_tools.py`; `test_mcp_wire_str_exc_closed_world_ast_ratchet` enforces exactly 54 authorized sites and 0 un-allowlisted sites across all 4 modules.
