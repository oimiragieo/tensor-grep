...
- **Fast Direct Writes for Rewrites (NEW).** Rewrite apply operations (`--apply`) use direct file writes with fused I/O (single read per file, no redundant stale checks), matching `sg`'s write strategy for maximum throughput.
