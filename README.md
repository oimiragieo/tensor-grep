...
release_docs_current_tag: v1.12.63

Latest tagged GitHub release: [`v1.12.63`](https://github.com/oimiragieo/tensor-grep/releases/tag/v1.12.63). GitHub assets and PyPI publication are verified by main CI before `publish-success-gate` passes.
Latest complete PyPI release: [`v1.12.63`](https://github.com/oimiragieo/tensor-grep/releases/tag/v1.12.63). This is also the latest complete release-asset distribution.
...
- Native CUDA correctness passed on the local 1GB/5GB RTX 4070 and RTX 5070 dogfood gates. Single-pattern cold grep still has no crossover, but post-`v1.12.63` local CUDA-native measurements show 100 fixed-string 1GB probes beating sequential `rg` end-to-end.
...
Active post-`v1.12.63` follow-up:
...
- post-`v1.12.63` dogfood keeps the near-term performance story on large-file CPU search, not GPU: public managed GPU remains `GpuSidecar` / unsupported, while CPU large-file rows are the strongest current win signal
