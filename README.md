...
release_docs_current_tag: v1.13.37

Latest tagged GitHub release: [`v1.13.37`](https://github.com/oimiragieo/tensor-grep/releases/tag/v1.13.37). GitHub assets and PyPI publication are verified by main CI before `publish-success-gate` passes.
Latest complete PyPI release: [`v1.13.37`](https://github.com/oimiragieo/tensor-grep/releases/tag/v1.13.37). This is also the latest complete release-asset distribution.
...
- PR #231 `fix: harden dogfood timeout reporting` shipped the release as merge commit `6525853 fix: harden dogfood timeout reporting` and release commit `c41d475 chore(release): v1.13.20 [skip ci]`.
- main CI run `26437847778` passed semantic-release, native asset publication, `publish-github-release-assets`, `publish-pypi`, and `publish-success-gate`; main dynamic/CodeQL run `26437847528` passed on the merge commit.
- GitHub release assets for `v1.13.20` include native CPU front doors, checksums, winget manifest, Homebrew formula, and publish instructions; `uvx --refresh-package tensor-grep --from tensor-grep==1.13.20 tg --version` reports `tensor-grep 1.13.20`.
- The release gives `tg dogfood` a bounded wrapper timeout, passes an incremental child readiness `--output`, preserves partial/current-check JSON on timeout, emits structured timeout failure evidence, and cleans up only the launched child process tree by PID.

What `v1.13.19` closed:

- PR #230 `fix: harden daemon response cache writes` shipped the release as merge commit `0c9155f fix: harden daemon response cache writes` and release commit `b9197a6 chore(release): v1.13.19 [skip ci]`.
- main CI run `26431129535` passed semantic-release, native asset publication, `publish-github-release-assets`, `publish-pypi`, and `publish-success-gate`; main dynamic/CodeQL run `26431129155` passed on the merge commit.
- GitHub release assets for `v1.13.19` include native CPU front doors, checksums, winget manifest, Homebrew formula, and publish instructions; `uvx --refresh-package tensor-grep --from tensor-grep==1.13.19 tg --version` reports `tensor-grep 1.13.19`.
- The release connects daemon response-cache writes and hits for top-level `context-render` / `edit-plan`, recovering sub-second warm context/edit-plan calls while preserving capped-session stale-file checks and complete-session added-file refresh.
...
- Native CUDA correctness passed on the local 1GB/5GB RTX 4070 and RTX 5070 dogfood gates. Single-pattern cold grep still has no crossover, but post-`v1.13.37` local CUDA-native measurements show 100 fixed-string 1GB probes beating sequential `rg` end-to-end.
...
Active post-`v1.13.37` follow-up:
...
- post-`v1.13.37` dogfood keeps the near-term performance story on large-file CPU search, not GPU: public managed GPU remains `GpuSidecar` / unsupported, while CPU large-file rows are the strongest current win signal
...
It also covers the broad generated-root scan and workspace-root scan guard: unbounded `tg search --files` roots that combine hidden/no-ignore-style scanning with generated, cache, or dependency directories, and unbounded searches against a parent containing multiple child project roots, must be scoped, bounded, or explicitly opted in with `--allow-broad-generated-scan`. AST/ruleset scans follow the same safety posture for direct temp, AppData, user-home, filesystem-root, generated/cache/dependency, and multi-project workspace roots; use a project/subdirectory `--path`, `--max-depth`, `--glob`, `--type`, or explicit `--allow-broad-generated-scan` before launching those broad walks.
...
- **Warm session contract:** Cached-session requests validate files captured in the session snapshot with size/mtime checks without walking the whole repository on every request. For agent loops, open a session once, start the localhost daemon, and send repeated daemon-routed edit-plan/context requests through that session so the first request can fill the daemon cache and later requests reuse it. Top-level native-provider `tg context-render` and `tg edit-plan` also reuse an already-running daemon by creating an implicit session keyed by root and `--max-repo-files` on the first daemon-routed request, so a later larger-budget request does not inherit a smaller cached scan. The `response_cache_*` daemon status counters are scoped to daemon-routed top-level/session `context-render` / `edit-plan` requests. Added-file discovery is reserved for `tg session refresh` and commands that opt into `--refresh-on-stale`, because broad added-file scans can dominate large workspace latency. Session edit-plan keeps graph ranking, test matching, and blast-radius metadata bounded to the selected context instead of re-expanding the whole cached map, and reports the blast-radius scope in `edit_plan_seed.blast_radius_scope`. `tg session list` and `tg session daemon status` discover nearby session scopes when the current directory has no direct session metadata, so parent/workspace shells do not misleadingly report an empty warm state. The localhost daemon keeps a short connect probe but uses a longer response timeout for edit-plan/context work.
...
- **Launcher Behavior:** The native Rust binary (`tg.exe`) acts as the primary front door, embedding AST search and fast-path text search. Unimplemented complex flags fall back to the Python sidecar. Plain `tg run` structural search and rewrite paths stay native when the native binary is available; ast-grep semantic read-only flags (`--selector`, `--strictness`, `--stdin`, and `--globs`) intentionally route to the Python AST workflow and `AstGrepWrapperBackend`, require an `ast-grep`/`sg` binary on PATH, and are reported by `tg doctor`. The Python wrapper (`python -m tensor_grep`) delegates structural and plain search commands back down to the native binary when available unless the command requires that Python AST workflow, guaranteeing uniform performance and path resolution without native/Python delegation loops. On Windows, `tensor-grep` intentionally rejects `PythonXY\Scripts\tg.exe` console-entrypoint shims when resolving that native path; use a release binary, an in-tree build, or `TG_NATIVE_TG_BINARY` when you need to force a specific native executable. A copied tensor-grep `tg.com` bridge outside `~/.tensor-grep/bin` must still discover the managed sidecar Python and managed native binary instead of the ambient Python install beside the bridge.
...
- **Native AST search and rewrite.** Simple `tg run` structural search plus rewrite planning, diff, apply, and verify stay native when the native binary is available. The richer ast-grep semantic read-only flags (`--selector`, `--strictness`, `--stdin`, and `--globs`) route through the Python AST workflow and external `ast-grep` wrapper instead of pretending to be native. PyPI wheels also expose Rust rewrite plan/apply through the PyO3 extension so simple CLI and MCP rewrite plan/apply paths work even when a standalone native `tg` binary is not installed.
...
To evaluate native CUDA search, your system must meet these requirements. End-to-end GPU routing is still benchmark-governed and host-specific; see [docs/gpu_crossover.md](docs/gpu_crossover.md) for the current measured line. Public managed GPU remains experimental until public binaries produce `NativeGpuBackend`, `sidecar_used = false`, direct `rg --json` 1GB/5GB route/correctness, and advanced many-pattern speed proof against the fair single-invocation `rg -F -e ... -e ...` baseline. Release-quality GPU promotion additionally requires managed NVIDIA front-door metadata in `tg-native-metadata.json` and a dispatch-only `public-gpu-proof.yml` run with `--public-managed-proof` where `public_managed_promotion_ready = true` and `public_gpu_proof = true`; a local CUDA-feature native build alone is not public GPU readiness.
