...
def _emit_broad_scan_refusal(
    ...
    *,
    ...
    file-cap, vendored, and generated ceilings (budget-remediable scan policy). The multi-project
    workspace-root call site overrides to ``workspace_root_refused`` on BOTH fields so an agent
    does not retry a parent refuse by raising ``--max-repo-files`` (v1.108.2 dogfood). ``error.code``
    remains the sibling signal for WHICH policy refused.
