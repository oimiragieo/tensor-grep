...
def _sentinel_insertion_index(search_args: list[str]) -> int | None:
    ...
    Unlike ``main._build_native_tg_search_command``, bootstrap receives a flat argv where
    trailing tg search flags (``--count-matches``, etc.) may follow paths. Inserting ``--``
    before the first non-dash positional would strand those flags after the sentinel and make
    the native parser treat them as paths.
    """
    if "--" in search_args:
        return None
...
def _first_dash_led_pattern_index_after_tg_flags(search_args: list[str]) -> int | None:
    ...
    Covers the CWE-88 injection shapes ``tg search --json -i -r`` (all-dash tail) and
    ``tg search --cpu -pattern src`` (long dash-led pattern + plain path).
    """
    index = 0
    while index < len(search_args):
        arg = search_args[index]
...
def _bootstrap_native_tg_search_argv(search_args: list[str]) -> list[str]:
    """Insert ``--`` before dash-led caller positionals for native delegation.

    Mirrors the CWE-88 / MCP-276 guard in ``main._build_native_tg_search_command``, adapted for
    bootstrap's flat argv: only dash-led patterns/paths need protection; trailing search flags must
    stay before the sentinel.
    """
    if "--" in search_args:
        return list(search_args)
...
def _run_native_tg_search(binary_name: str, search_args: list[str]) -> int:
    argv = _bootstrap_native_tg_search_argv(search_args)
    return _streaming_passthrough_returncode([binary_name, "search", *argv])
