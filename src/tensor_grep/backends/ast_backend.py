...
class AstBackend(ComputeBackend):
    ...
    def _get_parser(self, lang: str):
        ...
        try:
            if lang == "python":
                ...
                parser.set_language(tree_sitter.Language(tree_sitter_javascript.language(), "javascript"))
            else:
    ...
    def _ast_to_graph(self, root_node, source_bytes: bytes) -> Tuple[torch.Tensor, torch.Tensor, List[int]]:
        """
    ...
    def search(self, file_path: str, pattern: str, config: Optional[SearchConfig] = None) -> SearchResult:
        if not self.is_available():
            raise RuntimeError("AstBackend requires torch-geometric and tree-sitter to be installed.")
            
        lang = "python"
        if config and hasattr(config, "lang") and getattr(config, "lang"):
            lang = getattr(config, "lang")
