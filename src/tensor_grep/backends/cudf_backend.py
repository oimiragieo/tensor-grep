...
def _process_chunk_on_device(device_id: int, file_path: str, offset: int, size: int, pattern: str, config: Optional[SearchConfig] = None) -> list[MatchLine]:
    import cudf
    import rmm
    import re
...
class CuDFBackend(ComputeBackend):
    def __init__(self, chunk_sizes_mb: Optional[List[int]] = None):
                ...
                while offset < file_size:
                    for i, chunk_mb in enumerate(self.chunk_sizes_mb):
                        ...
                        future = executor.submit(
                            ...
                            i, file_path, offset, size, pattern, config
                        )
                        ...
                        offset += size
                        line_offset += (size // 50) # Rough estimate for fast numbering, true line offset is complex for chunked reads
