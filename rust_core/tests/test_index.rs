...
#[test]
fn test_tg_search_index_json_metadata() {
    let dir = tempdir().unwrap();
    write_corpus(dir.path());
    ...
    assert_eq!(meta["routing_backend"], "TrigramIndex");
    assert_eq!(meta["routing_reason"], "index-accelerated");
    assert_eq!(meta["total_matches"], 2);
}
