...
fn assert_repo_map_coverage(path: &Path, coverage: &CoverageExample) {
    ...
    assert_eq!(
        ...
        "parser-backed-refs-callers:csharp-go-java-javascript-python-rust-typescript+foundational-defs-imports-only:c-cpp-php",
        "{} coverage.symbol_navigation must stay parser-backed-refs-callers:csharp-go-java-javascript-python-rust-typescript+foundational-defs-imports-only:c-cpp-php",
        path.display()
    );
    assert_eq!(
