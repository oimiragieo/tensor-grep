...
fn test_rust_cpu_backend_fixed_string_count() {
    ...
    let count = backend.count_matches("ERROR", file_path.to_str().unwrap(), false, true, false).unwrap();
    assert_eq!(count, 2);
}
...
fn test_rust_cpu_backend_regex_count() {
    ...
    let backend = CpuBackend::new();
    let count = backend.count_matches("ERROR|CRITICAL", file_path.to_str().unwrap(), false, false, false).unwrap();
    assert_eq!(count, 2);
}
...
fn test_rust_cpu_backend_invert_count() {
    ...
    let count = backend.count_matches("INFO", file_path.to_str().unwrap(), false, true, true).unwrap();
