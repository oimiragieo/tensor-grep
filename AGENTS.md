...
- Current follow-up work is tracked in `docs/SESSION_HANDOFF.md`: keep release-native assets verified, preserve the managed installer fallback when assets are absent, keep sidecar and native front-door versions aligned after `tg upgrade`, design opt-in agent context capsule/token-economy surfaces without changing raw search contracts, and keep GPU/classify provider paths experimental until correctness, speed, and UX are proven.
