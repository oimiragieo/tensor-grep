...
- `tg search --json` is tensor-grep aggregate JSON, not ripgrep JSON Lines. `tg search --ndjson` is tensor-grep's flattened streaming row schema, not the rg event schema. Do not describe either as rg JSON compatibility unless the output mode is explicitly normalized by a parity harness.
