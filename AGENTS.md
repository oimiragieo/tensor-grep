...
release_docs_current_tag: v1.12.13

As of 2026-05-14, the current tagged release state is `v1.12.13`, and the latest complete public PyPI/release-asset distribution is also `v1.12.13`. The stable installer, release-native asset publication, managed-native `tg upgrade` refresh path, stale tensor-grep-owned `tg.com` bridge refresh after upgrade, native-front-door CLI parity fixes, Windows `.cmd` quoted-pattern launcher fix, native-first Windows PATH ordering, top-level validation-command contract, local default `classify`, classify provider provenance, fixed multi-pattern native CPU search, GPU scale benchmark correctness gates, launcher-route observability, benchmark launcher attribution, scoped GPU device probing, benchmark launcher warnings, opt-in `tg agent` Actionable Context Capsule, mixed-language capsule confidence/validation alignment, GPU benchmark recommendation hygiene, edit JSON/rollback safety, explicit language/file-name agent ranking, Windows validation-command quoting, docs/version governance, `$file` / `{file}` validation placeholder substitution, native CUDA correctness gates, ambiguous capsule alternative-target surfacing, root help-menu diagnostics, foreign launcher diagnostics, benchmark promotion-gate taxonomy, agent workflow benchmark governance, capsule alternative-confidence capping, generic provider-token `secrets-basic` regex rules, release-docs synchronization, release wheel Cargo prefetch retries, native GPU/search accuracy hardening, explicit Windows Python subprocess launcher repair, and agent capsule hardcase routing are released through `v1.12.13` GitHub assets and PyPI. Follow-up work should focus on context/session latency, GPU production viability, token economy, call-site evidence, AST parity roadmap, classify provider/cache UX, and keeping docs synchronized with release proof.

- Latest tagged release PR: #141 `fix: harden agent output budget hygiene`
- Latest tagged merge commit: `b601366 fix: harden agent output budget hygiene`
- Latest tagged release commit: `8d362de chore(release): v1.12.12 [skip ci]`
- Latest complete public release PR: #141 `fix: harden agent output budget hygiene`
- Latest complete public release commit: `8d362de chore(release): v1.12.12 [skip ci]`
- Latest merged fix commit: `b601366 fix: harden agent output budget hygiene`
...
- Main CI run `25948611239`: passed the pre-release matrix, semantic-release, PyPI wheel/sdist validation, `publish-github-release-assets`, `publish-pypi`, and `publish-success-gate`
- Main CodeQL run `25949010544`: passed on the `v1.12.12` release line
- PyPI pinned install: `uvx --refresh-package tensor-grep --from tensor-grep==1.12.13 tg --version` reports `tensor-grep 1.12.13`
- GitHub release: <https://github.com/oimiragieo/tensor-grep/releases/tag/v1.12.13>
...
- GitHub release assets: `tg-windows-amd64-cpu.exe`, `tg-linux-amd64-cpu`, `tg-macos-amd64-cpu`, checksums, winget manifest, Homebrew formula, and publish instructions are uploaded and verified on `v1.12.12`
