...
- Main CI run `25645819170`: passed the pre-release matrix, semantic-release, and `publish-github-release-assets`; `build-pypi-wheels (macos-latest)` failed on crates.io DNS, so `publish-pypi` was skipped and `publish-success-gate` failed as designed
...
- GitHub release assets: `tg-windows-amd64-cpu.exe`, `tg-linux-amd64-cpu`, `tg-macos-amd64-cpu`, checksums, winget manifest, Homebrew formula, and publish instructions are uploaded and verified on `v1.9.10`
- Public `v1.9.10` source/GitHub-asset dogfood: the release contains the v1.9.9 dogfood follow-ups, but PyPI remains at `tensor-grep==1.9.9` until the release-wheel retry follow-up publishes a replacement patch.
