...
As of 2026-05-08, the current released state is `v1.8.31`. The stable installer, release-native asset publication, managed-native `tg upgrade` refresh path, native-front-door CLI parity fixes, Windows `.cmd` quoted-pattern launcher fix, native-first Windows PATH ordering, top-level validation-command contract, local default `classify`, and GPU scale benchmark correctness gates are released and publicly dogfooded. Current follow-up work should focus on launcher-route observability, benchmark launcher attribution, agent context capsules/token economy, AST parity roadmap, GPU production viability, classify provider/cache UX, and keeping docs synchronized with release proof.
...
- Cold-path benchmark artifacts should include both `tg_launcher_mode` and `tg_launcher_command_kind`. Do not compare or market timings until native-exe, `.cmd` shim, `uv`, and Python-module routes are separated in the artifact.
...
- GPU benchmark correctness must treat no-match as a real comparator outcome. `rg` exit code `1` with empty output is valid when `tg` also returns no matches. GPU scale gates should include 1GB and 5GB rows and exact match/file-set correctness for every >=1GB GPU corpus before any GPU promotion claim.
