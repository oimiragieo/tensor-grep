...
- Main CI run `25829350863`: passed the pre-release matrix, semantic-release, PyPI wheel/sdist validation, `publish-github-release-assets`, `publish-pypi`, and `publish-success-gate`
...
- GitHub release assets: `tg-windows-amd64-cpu.exe`, `tg-linux-amd64-cpu`, `tg-macos-amd64-cpu`, checksums, winget manifest, Homebrew formula, and publish instructions are uploaded and verified on `v1.10.10`
- Public `v1.10.10` dogfood: release CI, assets, PyPI, `uvx --refresh-package tensor-grep --from tensor-grep==1.10.10 tg --version`, pinned managed installer, direct managed native `tg.exe`, fresh `cmd /c tg --version`, fresh `pwsh -NoProfile -Command "tg --version"`, and Python `subprocess.run(["tg", "--version"])` all verified `tg 1.10.10` after `tg repair-launcher --allow-foreign-rename` backed up the foreign Together CLI `tg.exe` and installed the verified managed native front door into that PATH slot.
