...
- GitHub release assets: `tg-windows-amd64-cpu.exe`, `tg-linux-amd64-cpu`, `tg-macos-amd64-cpu`, checksums, winget manifest, Homebrew formula, and publish instructions are uploaded and verified on `v1.9.8`
- Public `v1.9.8` dogfood: direct managed native `C:\Users\oimir\.tensor-grep\bin\tg.exe --version` reports `tg 1.9.8`; PyPI `tensor-grep==1.9.8` resolves; `tg update` advanced the managed sidecar and front door from `1.9.7` to `1.9.8`; the first run from the old updater left the tensor-grep-owned `tg.com` bridge stale at `1.9.7`, and invoking the released `1.9.8` updater refreshed that bridge so fresh `cmd` and unprofiled `pwsh` report `tg 1.9.8`; `tg doctor --json` reports `path_tg_first_launcher_kind = native-exe`, `fresh_shell_path_tg_first_launcher_kind = native-exe`, `rust_binary_version_status = matches`, and keeps unrelated first-PATH Together CLI `tg.exe` visible as a separate candidate rather than deleting or treating it as tensor-grep-owned stale state.
