...
1. **High-Density `llm` Render Profile (Skeleton Context Packer):** To directly address the token scalability bottleneck of `context-render` on 5,000+ file trees (which previously took >230s and blew out LLM context windows), we introduced the `llm` render profile. This profile performs AST-aware skeletonization of Python functions and classes, emitting only signatures and docstrings while omitting full function bodies. This maximizes "Contextual Information Density" and drastically reduces token overhead for AI edit-planning tools.
2. **Structural UX and AST Parity (Interactive Hardening):** Achieved 100% operational parity with `ast-grep` by harmonizing legacy `argparse` commands into the unified `Typer/Rich` framework and implementing high-fidelity structural controls. This includes a new `--interactive` confirm-apply loop for AST rewrites and a matched-node `--filter` regex boundary, ensuring `tensor-grep` is a first-class replacement for complex codebase refactoring.
