...
2. **Benchmark Governance and Provenance Hardening:** `tensor-grep` explicitly pins and isolates the repo-owned ripgrep bundle (`TG_RG_PATH`) across all benchmarking, testing, and Python backend boundaries, terminating ambient PATH drift and ensuring reliable, reproducible comparator lines.
