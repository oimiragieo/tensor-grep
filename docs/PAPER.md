...
The post-`v1.8.20` dogfood fix tightened the contributor/dev path instead of changing the benchmark story. Implicit native resolution now rejects stale in-tree standalone binaries under `rust_core/target/release` or `rust_core/target/debug` unless an agent explicitly opts in with `TG_NATIVE_TG_BINARY` or `TG_MCP_TG_BINARY`. `tg doctor --json` reports those rejected binaries under `skipped_native_tg_binaries`, uses `rust_binary_version_status = stale-skipped`, and keeps the acceleration backend on the embedded Rust extension when available. The same slice promotes `--format rg` from an internal-looking help string to the public exact ripgrep-style text-output mode, while preserving the existing ordering contract: root-scale raw output is semantic parity unless automation requests deterministic ordering with `--sort path`. This is dev-path safety and CLI contract clarity, not a search-speed claim.
