...
The remaining performance gap to raw `rg` on cold generic text search is now narrowed but still workload-specific. The current `v1.6.5` same-machine explicit native-binary artifact passes the frozen Windows regression gate with parity on all 10 rows, while raw `rg` still wins several individual cold rows. Repeated-query paths still show real room for index-driven gains. The native Rust AST backend now beats `sg` on search (0.754x ratio) and the recovered safe one-shot rewrite apply path (0.831x on 1,000 files / 50,000 rewrites in `artifacts/bench_ast_rewrite.json`), closing the AST apply regression that earlier plan-first routing reintroduced. The roadmap below prioritizes measured cold-path attribution over broad front-door widening.
...
**Native count cleanup recorded (2026-04-24):** A later native CPU pass found that direct-file `--cpu -c` could collect match payloads and then run a second count search to emit output. The accepted fix uses one count-only result for direct files, keeps count output unchanged, and updates the native CPU benchmark harness to set `TG_DISABLE_RG=1` for `tg --cpu` measurements so local bundled `rg` discovery cannot pollute native rows. On the current Windows artifact, the native-only `large_file_200mb_count` row measures `tg --cpu 0.050s` vs `rg 0.199s`. This is a native CPU result, not a new default cold-path claim.
