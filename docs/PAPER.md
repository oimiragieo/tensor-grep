...
Current accepted artifact line (`artifacts/patch_eval_demo/claude_skill_ab_limit12_samepack_bakeoff.json`):

* **baseline:** `mean_patch_applied_rate = 0.75`, `mean_validation_pass_rate = 0.75`, `mean_primary_span_hit_rate = 0.5`, mean wall clock `29.64s`
* **enhanced:** `mean_patch_applied_rate = 0.916667`, `mean_validation_pass_rate = 0.916667`, `mean_primary_span_hit_rate = 0.75`, mean wall clock `47.73s`

This is still a real product win on a harder corpus: `tensor-grep` materially improves final patch correctness for a real agent workflow on the accepted 12-task pack. However, the same benchmark also shows that the current enhanced path is slower and no longer perfect.
...
Once that 12-scenario corpus was in place, the user-style Claude A/B had to be rerun on the same expanded pack. The accepted same-pack artifact is `artifacts/patch_eval_demo/claude_skill_ab_limit12_samepack_bakeoff.json`, and it materially changes the headline from the older 10-scenario line. The enhanced setup still wins, but the gap narrows:

* **baseline:** `patch_applied = validation = 0.75`
* **enhanced:** `patch_applied = validation = 0.916667`

The remaining failures also sharpen the next product target:
...
That is a better world-class signal than the earlier 10-scenario `1.0 / 1.0` story. The accepted reading is no longer that the enhanced path "solves" the task family; it is that the enhanced path still beats plain Claude on the harder accepted corpus, and the new remaining miss gives the next concrete failure mode to drive.
