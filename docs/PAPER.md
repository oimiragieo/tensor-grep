...
The next accepted step was to stop comparing these contract variants one at a time and add a dedicated matrix runner (`benchmarks/run_claude_skill_ab_matrix.py`) that reuses the existing user-style A/B harness plus the existing patch bakeoff scorer. The first real matrix artifact (`artifacts/patch_eval_demo/claude_skill_ab_limit1_matrix.json`) confirms the current taxonomy on a controlled 2x2 slice. `standard/standard` remains the losing corner: the enhanced path falls back to `meta_question`, emits no patch, and scores `patch_applied = 0.0`. The other three corners all recover correctness on this probe, but they do not behave the same operationally. `standard/engage` succeeds while paying for an explicit task-engagement push (`first_tg_seconds = 24.15`, `post_edit_deliberation_seconds = 55.15`). `terse/engage` also succeeds, but still spends `43.36s` in post-edit deliberation. The interesting result is `terse/standard`: on this probe it keeps correctness, avoids the `meta_question` failure, records no `tg` call, and yields the lowest non-losing post-edit deliberation (`41.55s`). That is not enough to change the default yet, but it is the first measured sign that the contract space should now be explored with the matrix harness instead of more one-off prompt guesses. To keep that comparison surface stable, the repo now also includes a dedicated markdown renderer (`benchmarks/render_claude_skill_ab_matrix.py`) so future slices can be compared as artifacts rather than ad hoc prose summaries. Because broader slices are long-running on Windows, the matrix runner now checkpoints after each experiment and supports `--resume`, which is the operational change needed to make 3-5 task slices auditable instead of all-or-nothing.
