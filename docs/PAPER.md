...
To mitigate the ~5.17s penalty of falling back to pure Python when GPUs were unavailable or WSL contexts corrupted, we successfully bridged the `tensor-grep-rs` codebase back into the Python package using a **PyO3** native extension. By implementing a zero-allocation `memchr::memmem` fast-path for "count matches" (`-c`) operations directly within the PyO3 Rust extension, `tensor-grep` evaluates counting operations on massive files in just **0.080s**, obliterating `ripgrep`'s serialized **0.151s** execution time.
