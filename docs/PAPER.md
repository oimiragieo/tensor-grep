...
2. **Native AST and rewrite execution**
   The Rust AST backend is the accepted structural-search path and remains ahead of `sg` on search. Rewrite execution remains functional, but the latest apply benchmark failed the `sg` ratio gate and is tracked as a performance follow-up.
