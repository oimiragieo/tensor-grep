...
The next accepted step was to stop comparing these contract variants one at a time and add a dedicated matrix runner (`benchmarks/run_claude_skill_ab_matrix.py`) that reuses the existing user-style A/B harness plus the existing patch bakeoff scorer. The first real matrix artifact (`artifacts/patch_eval_demo/claude_skill_ab_limit1_matrix.json`) confirmed the taxonomy on a controlled 2x2 slice, but the more important accepted result is now the broader 3-task slice (`artifacts/patch_eval_demo/claude_skill_ab_limit3_matrix.json`). On that slice, `standard/standard` is still the only losing corner: it regresses to `patch_applied = 0.67`, `validation = 0.67`, and `meta_question_rate = 0.33`. The other three corners all hold full correctness (`patch_applied = validation = 1.0`), which means the default enhanced contract is now clearly leaving performance on the table. Among the three non-losing corners, `terse/engage` is currently the cheapest successful corner with `post_edit_deliberation_seconds = 41.11`, beating `standard/engage` (`48.56`) and `terse/standard` (`61.83`). That does not automatically justify a default flip, but it is the first broader-slice evidence that a combined task-engagement plus terse-output contract may be the next default worth probing. To keep that comparison surface stable, the repo now also includes a dedicated markdown renderer (`benchmarks/render_claude_skill_ab_matrix.py`) so future slices can be compared as artifacts rather than ad hoc prose summaries. Because broader slices are long-running on Windows, the matrix runner now checkpoints at record granularity inside each experiment, exposes that checkpoint path as an explicit helper, and supports `--resume`, which is the accepted operational change needed to make 5-task slices auditable instead of all-or-nothing.
