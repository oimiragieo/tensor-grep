...
>
> CI stabilization note (2026-04-27): the post-`v1.5.1` parity line is accepted as a correctness/release recovery slice, not a speed claim. CI exposed four concrete contract gaps after the high-density LLM render and parity work: a false PCRE2 fallback through RustCore when no PCRE2-capable `rg` exists, wrapper-only AST patterns silently falling into the native AST backend, interactive rewrite tests depending on ambient AST binaries, and Python fallback search drift around `--max-filesize`, replacement output, JSON/native delegation, and cached test state. The accepted fix removes the false PCRE2 fallback, makes wrapper-only AST syntax fail explicitly when the wrapper is unavailable, hermeticizes interactive apply coverage, preserves native delegation trigger modes, respects max-filesize in Rust fallback, and hardens YAML/runtime cache cleanup. Local native benchmark reruns preserved search parity but were not accepted as speed evidence because the Windows comparator drifted across runs; no benchmark row is promoted by this note.
