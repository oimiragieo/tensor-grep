...
**Native count cleanup recorded (2026-04-24):** A later native CPU pass found that direct-file `--cpu -c` could collect match payloads and then run a second count search to emit output. The accepted fix uses one count-only result for direct files, keeps count output unchanged, and updates the native CPU benchmark harness to set `TG_DISABLE_RG=1` for `tg --cpu` measurements so local bundled `rg` discovery cannot pollute native rows. On the current Windows artifact, the native-only `large_file_200mb_count` row measures `tg --cpu 0.076s` vs `rg 0.233s`. This is a native CPU result, not a new default cold-path claim.
