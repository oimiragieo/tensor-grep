...
We re-ran the benchmark suite on 2026-03-04 (commit `985e303`) from repository scripts and captured the output artifacts directly:
...
* **AST backend:** `function_definition` query completed in **0.013 seconds** (4 matches).
* **cyBERT backend:** classified 10,000 log lines in **0.153 seconds** (2,000 ERROR labels).
* **Torch backend:** exact-string query (`Database connection timeout`) completed in **0.510 seconds** (2,000 matches).
...
* **Regex Match:** ripgrep **0.465s** vs tensor-grep **0.700s**
* **Invert Match:** ripgrep **1.027s** vs tensor-grep **1.244s**
* **Context (`-C2`):** ripgrep **1.617s** vs tensor-grep **2.230s**
...
gantt
    ...
    section CPU (ripgrep)
    Native C DFA Evaluation :a1, 0, 0.465s
    
    section tensor-grep CLI (this run)
    tensor-grep Regex Match :a2, 0, 0.700s
```
...
* **Count Matches:** ripgrep **0.184s** vs tensor-grep **0.090s**

For other exact/fixed-string modes in this run:

* **Fixed Strings (`-F`):** ripgrep **0.426s** vs tensor-grep **0.697s**
* **Simple String Match:** ripgrep **0.424s** vs tensor-grep **0.664s**
...
gantt
    ...
    ripgrep Count              :a1, 0, 0.184s
    tensor-grep Count          :a2, 0, 0.090s
    
    section Other exact/fixed paths
    ripgrep Fixed Strings      :a3, 0, 0.426s
    tensor-grep Fixed Strings  :a4, 0, 0.697s
    tensor-grep Simple String  :a5, 0, 0.664s
```
