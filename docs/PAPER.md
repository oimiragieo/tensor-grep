...
We also added a workflow-focused AST startup harness (`run_ast_workflow_benchmarks.py`) to measure command-level orchestration instead of only single-pattern search latency. On the current local Windows host, the synthetic `tg scan --config sgconfig.yml` workflow now completes in **0.615 seconds** after batching `ast-grep` wrapper execution per rule instead of spawning one wrapper subprocess per file. This benchmark is intentionally small and deterministic so it can track AST workflow startup regressions without being dominated by huge wrapper-rule corpora.
