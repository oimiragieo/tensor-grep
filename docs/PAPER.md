...
* **Patch benchmark track:**
  repo-backed end-to-end patch scoring is now implemented on an accepted hard pack of `12` scenarios; Claude-enhanced clears the user-style pack, Copilot now has a completed same-pack comparative rerun at `0.5 / 0.5`, and Gemini remains operationally limited by slow timeout-heavy runs on this host
