...
The next accepted telemetry layer records time-to-first-useful-action. On the current probe artifact (`artifacts/patch_eval_demo/claude_skill_ab_limit1_first_action_trace.json`), both baseline and enhanced modify the target file almost immediately (`first_file_change_seconds = 0.095` baseline, `0.055` enhanced), but the first observable patch does not arrive until the end of the Claude run (`first_patch_seconds = 48.85` baseline, `87.72` enhanced). `first_tg_seconds` remains `null` for both because neither path actually invoked `tg` on this probe. The practical implication is sharper than before: once the task is engaged, the current latency gap is not file targeting latency. It is post-edit deliberation / patch-finalization time inside the agent.
