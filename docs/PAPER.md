...
We re-ran the benchmark suite on 2026-03-04 (commit `9ec43ed`) from repository scripts and captured the output artifacts directly:
...
* **AST backend:** `function_definition` query completed in **0.019 seconds** (4 matches).
* **cyBERT backend:** classified 10,000 log lines in **0.121 seconds** (2,000 ERROR labels).
* **Torch backend:** exact-string query (`Database connection timeout`) completed in **0.606 seconds** (2,000 matches).
...
* **Regex Match:** ripgrep **0.457s** vs tensor-grep **0.687s**
* **Invert Match:** ripgrep **0.980s** vs tensor-grep **1.241s**
* **Context (`-C2`):** ripgrep **1.871s** vs tensor-grep **2.161s**
...
gantt
    ...
    section CPU (ripgrep)
    Native C DFA Evaluation :a1, 0, 0.457s
    
    section tensor-grep CLI (this run)
    tensor-grep Regex Match :a2, 0, 0.687s
```
...
* **Count Matches:** ripgrep **0.133s** vs tensor-grep **0.082s**

For other exact/fixed-string modes in this run:

* **Fixed Strings (`-F`):** ripgrep **0.458s** vs tensor-grep **0.758s**
* **Simple String Match:** ripgrep **0.415s** vs tensor-grep **0.668s**
...
gantt
    ...
    ripgrep Count              :a1, 0, 0.133s
    tensor-grep Count          :a2, 0, 0.082s
    
    section Other exact/fixed paths
    ripgrep Fixed Strings      :a3, 0, 0.458s
    tensor-grep Fixed Strings  :a4, 0, 0.758s
    tensor-grep Simple String  :a5, 0, 0.668s
```
