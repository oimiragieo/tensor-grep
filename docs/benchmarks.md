...
- Public `v1.10.4` managed binary note: a public managed binary GPU request still routes through `GpuSidecar`, not `NativeGpuBackend`; `NativeGpuBackend` rows must come from a special native CUDA dogfood build or a release profile that explicitly ships CUDA-native front doors. These artifacts are not public GPU readiness until the public managed binary produces native correctness and speed wins.
