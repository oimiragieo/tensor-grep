...
## Latest Scripted Benchmark Snapshot (2026-04-27)
...
- On this host, the current `run_benchmarks.py` rerun preserved output parity across all rows, but `benchmarks/check_regression.py --baseline auto` failed against the frozen Windows baseline because both the `rg` comparator and `tg` timings drifted. Treat the cold-path rerun as correctness evidence, not an accepted speed baseline refresh.
...
- `tg search --cpu` is the stronger `tg` text path on the standard-corpus row, but default `tg search` is slightly faster on the 200MB row in this run
- `git grep --no-index` narrowly wins the 200MB host-local row; default `tg search` beats it on the standard-corpus row
...
| --- | --- | --- | --- | --- |
| cold_standard_corpus | 0.267s | 0.184s | 0.690x | PASS |
| large_file_200mb | 0.200s | 0.103s | 0.516x | PASS |
| large_file_200mb_count | 0.199s | 0.050s | 0.249x | PASS |
| many_file_directory | 0.204s | 0.048s | 0.233x | PASS |
...
| Metric | Value |
| --- | --- |
| `ast-grep` median | 0.180s |
| `tensor-grep` median | 0.135s |
| Ratio (`tg/sg`) | 0.754x |
| Gate (`<= 1.1`) | PASS |
...
| --- | --- | --- |
| Python | 0.858x | PASS |
| JavaScript | 0.755x | faster than `sg` |
| TypeScript | 0.761x | faster than `sg` |
| Rust | 0.793x | faster than `sg` |

### tensor-grep AST workflow startup (`run_ast_workflow_benchmarks.py`)

| Scenario | tensor-grep | Backend | Result |
| --- | --- | --- | --- |
| `tg run "def $FUNC():\n    $$$BODY" .` synthetic AST workflow | 0.026s | native | PASS |
| `python -m tensor_grep.cli.bootstrap scan --config sgconfig.yml` synthetic AST workflow | 0.355s | sidecar | PASS |
| `python -m tensor_grep.cli.bootstrap test --config sgconfig.yml` synthetic AST workflow | 0.596s | sidecar | PASS |
...
| --- | --- | --- | --- | --- |
| 1,000 files | 0.287s | 0.253s | 1,789,471 B | PASS |
| 5,000 files | 0.779s | 0.787s | 8,935,187 B | PASS |
| 10,000 files | 1.553s | 1.535s | 17,867,529 B | PASS |
...
| --- | ---: | ---: | ---: | ---: | --- |
| 10MB | 0.173s | 0.168s | 0.550s | 3.177x slower | no crossover |
| 100MB | 0.161s | 0.187s | 1.227s | 7.632x slower | no crossover |
| 500MB | 0.163s | 0.171s | timeout | n/a | FAIL |
| 1GB | 0.224s | 0.244s | timeout | n/a | FAIL |

The current native GPU benchmark reports `passed = false`, `gpu_auto_recommendation.should_add_flag = false`, and `crossover.exists = false` on this Windows host. Keep explicit GPU search manual-only until the end-to-end artifact shows both correctness and a real crossover.

### Python GPU/NLP sidecar benchmark (`run_gpu_benchmarks.py`)

| Corpus size | `rg` | `tg --cpu` | GPU 0 (`RTX 4070`) | GPU 1 (`RTX 5070`) |
| --- | ---: | ---: | ---: | --- |
| 1MB | 0.156s | 0.153s | 3.101s PASS | UNSUPPORTED |
| 10MB | 0.129s | 0.125s | 4.134s PASS | UNSUPPORTED |
| 100MB | 0.140s | 0.142s | 16.900s PASS | UNSUPPORTED |
| 1GB | 0.190s | 0.211s | timeout FAIL | UNSUPPORTED |

The Python sidecar artifact reports PyTorch `2.6.0+cu124`: RTX 4070 is operational but does not beat `rg`, while RTX 5070 / `sm_120` is dependency-bound with `no kernel image`. If the host has no operational CUDA device, this artifact should contain `status: "SKIP"`, `skipped: true`, empty timing rows, and `gpu_auto_recommendation.should_add_flag = false`.
