...
`benchmarks/run_agent_workflow_benchmarks.py` is the canonical workflow benchmark for the post-`v1.18.5` dogfood wedge: agent capsule routing plus safe edit-loop execution. It intentionally measures an agent-native workflow benchmark; not a cold exact-text speed claim.
...
- Latest post-`v1.18.5` dogfood tool comparison medians (3 samples): standard corpus `rg 0.087s`, `tg 0.097s`; 200MB file `rg 0.094s`, `tg 0.114s`; `git grep --no-index` remained much slower than both. Treat `rg` as the raw cold exact-text baseline.
