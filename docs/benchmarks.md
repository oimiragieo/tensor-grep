...
- Latest post-`v1.11.5` many-pattern public managed dogfood also failed the credibility bar. The fair baseline is `rg -F -e ... -e ...`, not sequential `rg`: 100 fixed no-match patterns over 1GB were `rg` multi-pattern: `0.071s`, `tg` CPU multi-pattern: `9.89s`, and `tg --gpu-device-ids 0` did not produce promotion evidence. The mixed 100-pattern row was `rg` multi-pattern: `0.121s` versus `tg` CPU multi-pattern: `9.97s`. Treat earlier local CUDA-native speedups against sequential `rg` as implementation history, not a public managed GPU claim.
