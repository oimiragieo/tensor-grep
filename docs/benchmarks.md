...
## Latest Scripted Benchmark Snapshot (2026-04-12)
...
| --- | --- | --- | --- |
| Simple String Match | 0.261s | 0.653s | Regression check PASS |
| Case-Insensitive Match | 0.305s | 0.596s | Regression check PASS |
| Regex Match | 0.345s | 0.706s | Regression check PASS |
| Invert Match | 0.561s | 0.635s | Regression check PASS |
| Count Matches | 0.094s | 0.145s | Regression check PASS |
| Context Lines (`-C2`) | 0.435s | 0.617s | Regression check PASS |
| Max Count (`-m 5`) | 0.021s | 0.188s | Regression check PASS |
| File Glob Filtering | 0.116s | 0.281s | Regression check PASS |
| Word Boundary | 0.136s | 0.289s | Regression check PASS |
| Fixed Strings (`-F`) | 0.100s | 0.302s | Regression check PASS |
...
path trails it across the published Windows cold-path rows in the current accepted artifact
`artifacts/bench_run_benchmarks.json`. The accepted read is therefore architectural, not
promotional: cold generic text search is not the main reason to adopt `tg`.
...
- `tg search --cpu` is the stronger `tg` text path on both published rows, but `rg` still wins both rows on this host
- default `tg search` beats `git grep --no-index` on the standard-corpus row, but trails it on the published large-file row
