...
## Latest Scripted Benchmark Snapshot (2026-03-17)
...
| Metric | Value |
| --- | --- |
| `ast-grep` median | 0.162s |
| `tensor-grep` median | 0.305s |
| Ratio (`tg/sg`) | 1.877x |
| Gate (`<= 3.0`) | PASS |
...
| --- | --- | --- |
| plan median | 0.387s | PASS |
| diff median | 0.469s | PASS |
| apply median | 1.905s | measured |
| `sg` apply median | 0.773s | comparison |
| `tg/sg` apply ratio | 2.464x | slower than `sg` on this host |
