...
Canonical population at that planning snapshot: **44 rows / 31 unfinished** = 7 READY (baseline/design), 14 BLOCKED (named dependencies/environment), 4 CEO_GATED, 6 DEMAND_GATED, 0 IN_FLIGHT; terminal dispositions: 8 SHIPPED and 5 RETIRED. The four CEO-gated owners remain #72/#77/#131/#169; #169 retains the spend gate. Mixed historical outcomes in their existing rows are preserved. These are dated historical counts; the live census is above.
...
- [ ] **AGT-01** — Status: BLOCKED; PR: none; Trigger: obtain reviewed real-repository holdout fixtures; Owner: Evaluation maintainer; `dd3c594` shipped the baseline metric, PR #1159 (`bb635941`) the fail-closed outcome join, and PR #1160 (`cc833b06`, v1.121.0) scorecard reporting without ranking changes; holdout curation is blocked on fixture authority, so this mixed disposition is not SHIPPED; prerequisite: fixture authority; plan: docs/plans/2026-09-07-agentic-quality-simplification.md#task-01; merged implementation history: PR #1159, PR #1160
- [ ] **AGT-02** — Status: READY; PR: none; Trigger: Owner: Session maintainer; reproduce residual S4 refresh race and label decision freshness; prerequisite: none; plan: docs/plans/2026-09-07-agentic-quality-simplification.md#task-02; PR #1136 merged its earlier implementation, but residual work remains READY for a new reviewed slice, not SHIPPED or IN_FLIGHT; merged implementation history: PR #1136
- [ ] **AGT-03** — Status: BLOCKED; PR: none; Trigger: Owner: Agent protocol maintainer; typed validators, stop conditions and canonical session prepare wire remain blocked on AGT-02's residual generation/freshness contract; prerequisite: AGT-02; plan: docs/plans/2026-09-07-agentic-quality-simplification.md#task-03; PR #1138 merged its earlier implementation, but remaining work has no open PR; merged implementation history: PR #1138
- [ ] **AGT-04** — Status: READY; PR: none; Trigger: Owner: Edit verification maintainer; Direct-main commit `9377ea4` shipped dependency-tree pruning + finite population budget (fail-closed on incomplete population); remaining before implementation clearance: Git-tracked identity as primary population source, reviewed design doc, symlink/junction confined-primitive controls, full valid/unrelated/no-op/malformed/legacy control-matrix re-run; plan: docs/plans/2026-09-07-agentic-quality-simplification.md#task-04; READY denotes baseline/design work, not implementation clearance
- [ ] **AGT-05** — Status: READY; PR: none; Trigger: Owner: Ranking maintainer; Direct-main commit `bb1aa70` shipped a confidence-calibration REPORT (fixed bins + selective-accuracy-vs-coverage curve, additive, no scorer/ranking change); remaining before implementation clearance: DEVELOPMENT-data paraphrases + holdout-safe hardcases, feature tracing for wrong-primary causes, one-hypothesis-at-a-time scorer ablation with a no-change control, order-pin re-verification; plan: docs/plans/2026-09-07-agentic-quality-simplification.md#task-05; READY denotes baseline/design work, not implementation clearance
- [ ] **P9** — Status: BLOCKED; PR: none; Trigger: Owner: Session maintainer; `docs/design/2026-09-07-cache-ownership.md` shipped the component-ownership map and formalized `semantic_index.py`'s RETAIN/library-only disposition (guarded by `tests/unit/test_semantic_index_dormant_disposition.py`); remaining zero-redundant-warm-map-builds prepare-from-map extraction and generation-safe publication are blocked on AGT-02 and AGT-03's residual contracts, both touching shared `prepare_service.py`/`session_store.py`; prerequisite: AGT-02 and AGT-03; existing daemon gates preserved; plan: docs/plans/2026-09-07-agentic-quality-simplification.md#task-06; PR #1140 merged earlier implementation but no PR is open for the remaining scope; merged implementation history: PR #1140
- [ ] **P13** — Status: READY; PR: none; Trigger: Owner: Architecture maintainer; Direct-main commit `e7ee4f0` shipped the "freeze import edges" half of Task 07: `core/import_edges.py` (ast-based, dependency-free, correctly resolves relative cross-package imports per Codex Luna audit fix) plus `docs/design/2026-09-07-import-edges-baseline.json` and `tests/unit/test_import_edges_baseline.py` pinning the current cli/core/backends/io graph, including pre-existing violations (core->cli, backends->cli) not yet enforced against; remaining before implementation clearance: adopting import-linter itself (P13's original scope, still open), the CLI/MCP shared-service extraction (moving code, not just freezing the graph), and preserving public wrappers/monkeypatch behavior during that extraction per file-split-package-pattern; prerequisite: none; plan: docs/plans/2026-09-07-agentic-quality-simplification.md#task-07; READY denotes baseline/design work, not implementation clearance
- [x] **AGT-06** — Status: SHIPPED; PR: none; Trigger: Owner: CLI maintainer; Direct-main commit `390c39f` shipped the full Task 08 scope: `io/root_probe.iter_top_level_vendored_dirs` shared between bootstrap's short-circuit and main.py's sorted-diagnostics adapters, frozen fixture-table parity test locked pre-refactor baseline and re-confirmed post-refactor (174/174 + 288/288 wider sweep), Codex Luna (read-only) AUDIT_CLEAR; no remaining Task 08 checkboxes; prerequisite: none; plan: docs/plans/2026-09-07-agentic-quality-simplification.md#task-08; Direct-main SHA: 390c39f; CLOSED 2026-09-12: the row said "no remaining Task 08 checkboxes" yet sat at READY because the canonical-index grammar required a literal `PR #NNN` for SHIPPED and this work landed as a direct commit with no PR -- a permanent FALSE READY that invites re-dispatch. The grammar now accepts a `Direct-main SHA:` receipt for SHIPPED only (tests/unit/test_backlog_tracker_truth.py), so the status can finally state what the prose already did; reopen on a new failing Task 08 receipt
- [ ] **AGT-07** — Status: READY; PR: none; Trigger: Owner: Result-contract maintainer; Direct-main commit `22cb25c` shipped the typed-record + parity-proof half of Task 09: `core/completeness.CompletenessEvidence` (enum-typed ScanState/Cause/RetryKind) round-trips byte-for-byte with `unified_incomplete_envelope()` across the frozen truth table + 2 mutation controls; Codex Luna audit round 1 caught a real cause=None-vs-"unknown" round-trip bug, fixed, round 2 AUDIT_CLEAR; remaining READY slice: migrate one producer/consumer to the typed record and prove old/new projections agree on every truth-table row before migrating the sibling; prerequisite: none; plan: docs/plans/2026-09-07-agentic-quality-simplification.md#task-09; PR #1137 merged earlier implementation, but no PR is open for the remaining scope; merged implementation history: PR #1137
- [ ] **AGT-08** — Status: BLOCKED; PR: none; Trigger: Owner: Retrieval maintainer; Direct-main commit `c366a34` shipped the core-primitive half of Task 11: `core/retrieval_fusion.reciprocal_rank_fusion_explained` returns the exact real per-leg terms (fuzz-proven byte-identical order to the plain function across 25 random shapes, both combine modes); Codex Luna audit round 1 caught two real defects (duplicate-chunk-id divergence, silent duplicate-leg-name overwrite), both fixed, round 2 AUDIT_CLEAR; remaining CLI/MCP and BM25+dense wiring plus path-boost/fallback-reason legs are blocked on AGT-01 measurement controls; prerequisite: AGT-01 holdout-safe controls; plan: docs/plans/2026-09-07-agentic-quality-simplification.md#task-11; PR #1139 merged earlier implementation, but no PR is open for the remaining scope; merged implementation history: PR #1139
- [ ] **P12** — Status: READY; PR: none; Trigger: Owner: Retrieval maintainer; Direct-main commit `1f6f4b0` shipped the per-phase measurement instrument for Task 12: `core/retrieval_dense.DenseStageTimings` + `DenseIndex.query_with_timings()` (corpus-encode, query-encode, score, sort timed separately; `query()` delegates to it so ranking is provably unchanged) plus `benchmarks/profile_dense_stages.py`, a min-of-N synthetic-corpus harness that skips cleanly without the optional model2vec extra; remaining before implementation clearance: model/corpus reuse-by-identity, bounded top-k selection vs full sort, and the actual float16/int8/ONNX comparison arm; prerequisite: AGT-01 and AGT-08; plan: docs/plans/2026-09-07-agentic-quality-simplification.md#task-12; READY denotes baseline/design work, not implementation clearance
- [ ] **P14** — Status: READY; PR: none; Trigger: Owner: Symbol intelligence maintainer; Direct-main commit `3a69430` shipped Task 13's inventory checkbox: docs/design/2026-09-07-p14-provenance-census.md census confirms defs/refs/callers and mcp_symbol_tools.py carry no per-fact provenance today (only a per-file/per-target string exists at repo_map.py:2416), and removed/hedged the unsubstantiated first-to-market claim per the audit's own research-correction note, pinned by tests/unit/test_symbol_fact_provenance.py; remaining before implementation clearance: the actual provenance envelope + method/freshness/confidence_kind fields on defs/refs/callers, and the LSP precise-provider pilot; prerequisite: AGT-01/02/07 shipped baseline primitives only, with AGT-01 fixture authority BLOCKED and AGT-02/07 residual scopes READY; plan: docs/plans/2026-09-07-agentic-quality-simplification.md#task-13; READY denotes P14 baseline/design work, not implementation clearance
- [x] **P7** — Status: RETIRED; PR: none; Trigger: Direct-main commit `251cbaf` closed Task 14 as research, no code: docs/design/2026-09-07-p7-cache-coverage-research.md census confirms mtime-aware in-process caching already covers parser/query/source (`repo_map_cache._mtime_aware_cache`, `ast_scan.py`'s per-scan `source_cache`, 13 other cache sites); P7's literal ask (a persistent disk `.tensor-grep/ast_cache/`) is redundant with P9's daemon-based warm-cache design and reintroduces the exact cross-process invalidation-desync risk P7's own acceptance bar warned against; no measured gap found, none manufactured; reopen only under P9's scope if a real gap surfaces once P9 ships; plan: docs/plans/2026-09-07-agentic-quality-simplification.md#task-14
- [ ] **P10** — Status: READY; PR: none; Trigger: Owner: Evidence maintainer; Direct-main commit `0f5a1f4` shipped Task 15's naming/correctness half: `_rollback_risk_from_blast_radius`'s test-count risk factor renamed from `coverage_factor` to `test_association_factor` to disclose it is a static import-scan count, not measured runtime coverage; numeric output unchanged (parity-proven); bind observed execution to revision (bounded process/output + replay controls) remains open; prerequisite: AGT-02/03/04 and P14; F5/F6 gates preserved; plan: docs/plans/2026-09-07-agentic-quality-simplification.md#task-15; READY denotes baseline/design work, not implementation clearance
- [ ] **P15** — Status: READY; PR: none; Trigger: Owner: Documentation maintainer; Generate capability facts and correct unsupported public prose; prerequisite: none for baseline facts; plan: docs/plans/2026-09-07-agentic-quality-simplification.md#task-16; drift-check slice shipped (`tests/unit/test_public_docs_governance.py::test_tool_comparison_language_coverage_facts_are_generated_not_hand_typed`); README tagline positioning and CI wiring remain open; READY denotes baseline/design work, not implementation clearance
...
- [x] **#48** — Status: RETIRED; PR: none; Trigger: GitHub issue #48 closed "not planned" 2026-08-24 applying the standing 5/5 council verdict (accept the shipped hybrid, do not build the larger native-front-door rewrite absent a concrete driving complaint); reopen if that rewrite is later authorized on its own merits
...
- [ ] **F8** — Status: BLOCKED; PR: none; Trigger: Tasks 12-13 workspace program blocked on rust_core/src/main.rs + tests/e2e routing parity (shared-box ban → CI/cloud); re-derived 2026-08-30: `rust_core/src/path_domain.rs` does NOT exist on origin/main (`git cat-file -e origin/main:rust_core/src/path_domain.rs` fails; `grep -rln "path_domain\|PathDomain" rust_core/src/` returns nothing) — real touch points are main.rs routing until a design names workspace APIs
- [ ] **MCP-SURFACE** — Status: BLOCKED; PR: none; Trigger: Task 4 MCP surface disclosure sequenced after Task 2C; **CORRECTED 2026-09-12 (re-derived, not re-stamped):** live `_TG_MCP_SERVER_CONTRACT_VERSION` is **1.8.0** (Python const in `src/tensor_grep/cli/mcp_server.py`, grep the symbol -- the previous `:188` anchor had drifted; zero references in `rust_core/src/`). The bump landed in `5b8c85e` (`feat(mcp): standardize incompleteness envelope across MCP tools (P3)`, PR #1135), NOT via Task 2C. The recorded blocker -- "Task 4 plans 1.8.0→1.9.0 and must not bump from a nonexistent base" -- is therefore SATISFIED: 1.8.0 exists. This row's Task-2C sequencing premise is dead and the row needs re-triage on its own merits before any 1.9.0 work; it is not rust/shared-box blocked
...
closing **AGT-06**). Shipped: **HUNT-1/2/3/5**
(`5331dc9`/`8ae32db`/`bb1d92c`/`b93e8e8`). **HUNT-4 OPEN** (plan hash `b86a7e47`, one of two
clean council rounds banked, see `docs/BACKLOG.md`). canonical index `2026-09-07.1` (unchanged).
2026-09-12 paragraph below is untouched; read as of that date.

Last reconciled: **2026-09-12** (session closeout: stamp `post-**v1.119.8**`, PyPI-verified 4/4 BY FILENAME for each of v1.119.5/.6/.7/.8 -- 3 wheels (macosx_11_0_arm64, manylinux_2_39_x86_64, win_amd64) + sdist, read from the PyPI releases map, not `info.version` and not from a log). canonical index `2026-09-07.1`. Reconciles the 2026-09-12 change-proposal-audit triage: 8 PRs merged (#1142 ast-grep metavariable guard + YAML --lang twin, #1143 NLP keyword auto-route default-OFF, #1144 durable fallback_reason on engine swap, #1145 JS/TS tsconfig scope-gap disclosure, #1146 [scan] extra, #1147 edit-verify repo_root binding, #1148 AGT-04 independent security review, #1149 editor-plane graph_completeness derived) plus 5 direct-main docs/test commits. Canonical rows AGT-02/03/07/08/P9 moved READY->IN_FLIGHT with their implementation-PR receipts; strategic rows P1/P2/P3/P4 closed against their own receipts; MCP-SURFACE and MCP-LEAN-DEFAULT corrected to the live contract 1.8.0 (the Task-2C base-version fence was already satisfied by 5b8c85e). Open PRs at stamp time: #1141 (native root-flag parity), #1150 (SHIPPED accepts a Direct-main SHA receipt; closes AGT-06), #1151 (dogfood harness). Blocked with receipts in `.orchestrator/state.json`: C10 bare-reserved-command fallthrough, C12 RRF wiring (AGT-01 gate), C15 tg_prepare MCP tool (Task 4 owns the 1.9.0 bump), and a NEW finding -- `--version` diverges between the native door (`tg X.Y.Z`) and the Python door/published wheel (`tensor-grep X.Y.Z`), which routing-parity never compares. **Task 2A RED remains correctly blocked** (untouched this session -- not advanced, not GREEN). CEO packet: `docs/audits/2026-08-13-ceo-backlog-update.md` (historical closeout; newer dated audits exist but this file remains the governance pin). CEO gates unchanged.
...
**Public product:** `v1.113.5` on PyPI/GitHub (4/4 per-artifact verified 2026-08-23; unchanged at reconcile). SEC-001 (`fix(security)` #1122 / `a77a150`) merged 2026-08-30; await main CI publish before treating the patch as shipped on PyPI.
...
post-**v1.113.6**, PyPI-verified 2026-09-03 (`v1.113.6` live on PyPI). Reconciles 2026-09-03 session: **HYGIENE-FORMAT CLOSED** (1,003/1,003 files formatted repo-wide); **GOVERNANCE-DOC-SIZE-RATCHET SHIPPED** (`scripts/check_governance_doc_size.py` and `tests/unit/test_governance_doc_size_ratchet.py` enforce budget ceilings); **CONFIDENCE-HYGIENE SHIPPED** (empty/whitespace downgrade reasons stripped in `agent_capsule_confidence.py`); **SYMBOL-PRIORITY-PREFERENCE SHIPPED** (`_prefer_public_implementation_over_private_helper` resolves `_add` lexical trap in `agent_capsule_builder.py` and `agent_capsule_confidence.py`, verified in `test_agent_capsule_symbol_priority.py` and `test_agent_capsule_hardcases.py`). Open PRs: **0**. CEO gates unchanged.

post-**v1.113.5**, PyPI-verified 2026-08-30 (4/4 BY FILENAME unchanged from v1.113.3 control: `macosx_11_0_arm64` + `manylinux_2_39_x86_64` + `win_amd64` wheels + sdist). Reconciles 2026-08-30 session: **SEC-001 SHIPPED** — PR #1122 / `a77a150` (`fix(security): bootstrap native search -- sentinel for dash-led args`, CWE-88 argv hygiene in `bootstrap_native_argv.py`); **ENV-SYNC CLOSED** — `uv sync --frozen --extra dev` restored dev toolchain, `tg doctor --json` reports `rust_binary_version_status: matches`, routing parity **68/68** twice on `origin/main` at `a77a150`. Canonical rows **F8** and **MCP-SURFACE** triggers corrected (no `path_domain.rs`; MCP contract is Python-only at `mcp_server.py:188`). Open PRs: **0**. Next ranked slice: HYGIENE-FORMAT (15 markdown files fail `ruff format --preview`). CEO gates unchanged.
...
post-**v1.111.6**, PyPI-verified 2026-08-22, dogfooded on the published wheel. Reconciles the
2026-08-21/22 session: v1.111.3→v1.111.6 published 4/4 each, closing a tagged-but-unpublished
gap; PYPI-SIZE-CAP cleared (713→287 releases); several product/CI fixes shipped. Full detail:
git history at this line, or `docs/audits/` for that date. CEO gates unchanged, no status flips.
(Trimmed 2026-09-13 to make room under the doc's own size ratchet -- see git blame for the
original paragraph if the detail is needed.)
...
## Historical PR snapshot (2026-09-05)

The dated table and stale-state lesson are preserved in
[the 2026-09-05 PR snapshot](audits/2026-09-05-task-board-pr-snapshot.md). It is not a live queue;
the 2026-09-22 open-PR result is at the top of this board.
...
## P3 — strategic / positioning (2026-09-04 competitive analysis & roadmap)

- [x] **Fuse Semantic Dense Retrieval into `tg prepare` / `tg agent` (P0)** — Currently `prepare` and `agent` rely solely on lexical term overlap, AST definitions, import graphs, and file centrality; they do NOT fuse `retrieval_dense` or `retrieval_fusion` from `tg find`. Wire dense semantic retrieval as a hybrid signal / fallback when lexical confidence is low (<0.6) to eliminate natural language task vocabulary mismatches.
- [x] **Git Diff-Aware Blast Radius / PR Impact (`tg diff-impact`) (P1)** — Competitors (Gortex `pr_risk`, GitNexus `impact`) offer diff-level blast radius. Add `tg diff-impact [REF]` to parse modified symbols across a git diff/revision, compute the union blast radius floor, identify impacted tests, and output a review risk score. **SHIPPED 2026-09-12 reconcile:** PR #1128, merged SHA `7d2baa5`, released in `v1.116.0` (run `33995069360`); see docs/BACKLOG.md's P1 receipt.
- [x] **Deepen Language Coverage in `LANGUAGE_REGISTRY` (5 -> 10 Deep Languages) (P2)** — Upgrade the 5 foundational languages (Java, C#, C, C++, PHP) from regex caller heuristics to full tree-sitter AST-verified references and callers to defend against Gortex's ~30-language deep tier. **SHIPPED 2026-09-12 reconcile:** PR #1129, squash SHA `c762b1c`. Re-derived live, not hand-counted: `repo_map._symbol_navigation_descriptor()` returns `parser-backed-refs-callers:c-cpp-csharp-go-java-javascript-php-python-rust-typescript+foundational-defs-imports-only:` -- 10 parser-backed, foundational tier EMPTY.
- [x] **Standardize MCP Incompleteness Protocol Envelope (P3)** — Expose a unified additive JSON field `incomplete: {"status": bool, "cause": str, "budget_remediable": bool}` across all tool responses in `mcp_server.py` alongside existing fields, establishing `tg`'s fail-closed incompleteness contract as the standard for MCP client agents. **SHIPPED 2026-09-12 reconcile:** PR #1135, squash SHA `5b8c85e`, released as `v1.118.0` (PyPI 4/4 by filename, `release-tag-smoke` green); this is the same commit that bumped `_TG_MCP_SERVER_CONTRACT_VERSION` to 1.8.0.
- [x] **Front-Door Positioning & Dynamic Language Table Realignment (P4)** — Realign `docs/tool_comparison.md` and `README.md` to lead with `tg prepare` and the 4-element edit readiness comparison table, demote cold grep speed benchmarks to an engine appendix, and dynamically generate the language tier table from `LANGUAGE_REGISTRY`. **ACCEPTANCE MET 2026-09-12 reconcile:** residual README-tagline scope was split off as **P15**, which remains open; see docs/BACKLOG.md's P4 entry.
