...
- [ ] **AGT-06** — Status: READY; PR: none; Trigger: Owner: CLI maintainer; Direct-main commit `390c39f` shipped the full Task 08 scope: `io/root_probe.iter_top_level_vendored_dirs` shared between bootstrap's short-circuit and main.py's sorted-diagnostics adapters, frozen fixture-table parity test locked pre-refactor baseline and re-confirmed post-refactor (174/174 + 288/288 wider sweep), Codex Luna (read-only) AUDIT_CLEAR; no remaining Task 08 checkboxes; prerequisite: none; plan: docs/plans/2026-09-07-agentic-quality-simplification.md#task-08; READY denotes baseline/design work, not implementation clearance -- this canonical status field stays a distinct closure gate from the direct-main commit itself
