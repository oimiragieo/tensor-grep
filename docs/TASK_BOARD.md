...
- [ ] **AGT-01** — Status: IN_FLIGHT; PR: PR #1160; Trigger: latest implementation PR is #1160 and ordered history below retains #1159; obtain real repository holdout fixtures with independently reviewed known-good patches; Owner: Evaluation maintainer; `dd3c594` shipped the baseline wrong-confident-primary metric, PR #1159 (`bb635941`) shipped the fail-closed six-field outcome join, and PR #1160 (`cc833b06`, v1.121.0) wired verified outcomes into scorecards without changing ranking scores; remaining scope is holdout manifest curation and stays blocked on trustworthy fixtures, so this mixed disposition is not SHIPPED; prerequisite: fixture authority; plan: docs/plans/2026-09-07-agentic-quality-simplification.md#task-01; Implementation PRs: PR #1159, PR #1160
- [ ] **AGT-02** — Status: IN_FLIGHT; PR: PR #1136; Trigger: Owner: Session maintainer; Reproduce residual S4 refresh race and label decision freshness; prerequisite: none; plan: docs/plans/2026-09-07-agentic-quality-simplification.md#task-02; IN_FLIGHT denotes the implementation PR merged to main; the remaining scope named above is not implementation clearance and closure needs a separate closure PR; Implementation PRs: PR #1136
- [ ] **AGT-03** — Status: IN_FLIGHT; PR: PR #1138; Trigger: Owner: Agent protocol maintainer; Typed validators, stop conditions and canonical session prepare wire; prerequisite: AGT-02 generation contract; plan: docs/plans/2026-09-07-agentic-quality-simplification.md#task-03; IN_FLIGHT denotes the implementation PR merged to main; the remaining scope named above is not implementation clearance and closure needs a separate closure PR; Implementation PRs: PR #1138
...
- [ ] **P9** — Status: IN_FLIGHT; PR: PR #1140; Trigger: Owner: Session maintainer; `docs/design/2026-09-07-cache-ownership.md` shipped the component-ownership map and formalized `semantic_index.py`'s RETAIN/library-only disposition (guarded by `tests/unit/test_semantic_index_dormant_disposition.py`); remaining before implementation clearance: zero-redundant-warm-map-builds prepare-from-map extraction and generation-safe publication, both touching shared `prepare_service.py`/`session_store.py`; prerequisite: AGT-02 and AGT-03; existing daemon gates preserved; plan: docs/plans/2026-09-07-agentic-quality-simplification.md#task-06; IN_FLIGHT denotes the implementation PR merged to main; the remaining scope named above is not implementation clearance and closure needs a separate closure PR; Implementation PRs: PR #1140
...
- [ ] **AGT-07** — Status: IN_FLIGHT; PR: PR #1137; Trigger: Owner: Result-contract maintainer; Direct-main commit `22cb25c` shipped the typed-record + parity-proof half of Task 09: `core/completeness.CompletenessEvidence` (enum-typed ScanState/Cause/RetryKind) round-trips byte-for-byte with `unified_incomplete_envelope()` across the frozen truth table + 2 mutation controls; Codex Luna audit round 1 caught a real cause=None-vs-"unknown" round-trip bug, fixed, round 2 AUDIT_CLEAR; remaining before implementation clearance: migrate one producer/consumer to the typed record and prove old/new projections agree on every truth-table row before migrating the sibling; prerequisite: none; plan: docs/plans/2026-09-07-agentic-quality-simplification.md#task-09; IN_FLIGHT denotes the implementation PR merged to main; the remaining scope named above is not implementation clearance and closure needs a separate closure PR; Implementation PRs: PR #1137
- [ ] **AGT-08** — Status: IN_FLIGHT; PR: PR #1139; Trigger: Owner: Retrieval maintainer; Direct-main commit `c366a34` shipped the core-primitive half of Task 11: `core/retrieval_fusion.reciprocal_rank_fusion_explained` returns the exact real per-leg terms (fuzz-proven byte-identical order to the plain function across 25 random shapes, both combine modes); Codex Luna audit round 1 caught two real defects (duplicate-chunk-id divergence, silent duplicate-leg-name overwrite), both fixed, round 2 AUDIT_CLEAR; remaining before implementation clearance: wire this into `tg search`'s CLI/MCP output and the production BM25+dense call sites, plus path-boost/fallback-reason legs the audit named; prerequisite: none for the primitive, AGT-01 measurement controls before the wired feature; plan: docs/plans/2026-09-07-agentic-quality-simplification.md#task-11; IN_FLIGHT denotes the implementation PR merged to main; the remaining scope named above is not implementation clearance and closure needs a separate closure PR; Implementation PRs: PR #1139
...
- [ ] **P14** — Status: READY; PR: none; Trigger: Owner: Symbol intelligence maintainer; Direct-main commit `3a69430` shipped Task 13's inventory checkbox: docs/design/2026-09-07-p14-provenance-census.md census confirms defs/refs/callers and mcp_symbol_tools.py carry no per-fact provenance today (only a per-file/per-target string exists at repo_map.py:2416), and removed/hedged the unsubstantiated first-to-market claim per the audit's own research-correction note, pinned by tests/unit/test_symbol_fact_provenance.py; remaining before implementation clearance: the actual provenance envelope + method/freshness/confidence_kind fields on defs/refs/callers, and the LSP precise-provider pilot; prerequisite: AGT-01/02/07 (all shipped); plan: docs/plans/2026-09-07-agentic-quality-simplification.md#task-13; READY denotes baseline/design work, not implementation clearance
...
Last reconciled: **2026-09-13** (stamp `post-**v1.119.14**` ONLY -- keeps the freshness gate
inside tolerance, NOT a content re-audit; prior stamp `post-v1.119.8` went 6 releases stale from
this session's #1154/#1141/#1150 merge cascade, closing **AGT-06**). Shipped: **HUNT-1/2/3/5**
