...
- [ ] **MCP-SURFACE** — Status: BLOCKED; PR: none; Trigger: Task 4 MCP surface disclosure sequenced after Task 2C; live `_TG_MCP_SERVER_CONTRACT_VERSION` is 1.7.0 at `src/tensor_grep/cli/mcp_server.py:188` (Python const; zero references in `rust_core/src/`); Task 4 plans 1.8.0→1.9.0 and must not bump from a nonexistent base — row is Python-sequenced, not rust/shared-box blocked
