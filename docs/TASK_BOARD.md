...
- [ ] **Git Diff-Aware Blast Radius / PR Impact (`tg diff-impact`) (P1)** — Competitors (Gortex `pr_risk`, GitNexus `impact`) offer diff-level blast radius. Add `tg diff-impact [REF]` to parse modified symbols across a git diff/revision, compute the union blast radius floor, identify impacted tests, and output a review risk score.
