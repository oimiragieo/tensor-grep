...
## Current post-`v1.11.0` GPU dogfood Read

The post-`v1.11.0` native CUDA work changes the GPU read: single-pattern cold grep is still not a promotion story, but high-intensity fixed-string scans now have a credible native CUDA lane.
...
The historical artifact remains useful optimization history, but the post-`v1.11.0` decision above is the current contract.
