...
## Current post-`v1.11.4` GPU dogfood Read

The post-`v1.11.4` dogfood keeps public GPU not promotion-ready. Single-pattern cold grep is still not a promotion story, and public managed many-pattern search is also not credible versus fair `rg -F -e ... -e ...` multi-pattern search.
...
The historical artifact remains useful optimization history, but the post-`v1.11.4` decision above is the current contract.
