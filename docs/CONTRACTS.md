...
- Edit validation command templates support `$file` and `{file}` placeholders. The placeholders are replaced with the selected target path before command parsing/execution; examples should quote the placeholder, such as `python -m py_compile "$file"`, so Windows paths with spaces remain one argv item.
