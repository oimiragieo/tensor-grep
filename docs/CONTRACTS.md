...
release_docs_current_tag: v1.10.1
...
- If the first PATH `tg` reports a version string that is not tensor-grep-shaped (`tg ...` or `tensor-grep ...`), `tg doctor --json` should classify it as `foreign`, set `path_tg_first_is_foreign` or `fresh_shell_path_tg_first_is_foreign`, and emit a warning plus remediation field such as `path_tg_foreign_warning`, `path_tg_foreign_remediation`, `fresh_shell_path_tg_foreign_warning`, and `fresh_shell_path_tg_foreign_remediation`. Foreign launchers are not tensor-grep-owned stale launchers: readiness should fail with explicit remediation, but installer/doctor code must not delete or overwrite unrelated tools.
