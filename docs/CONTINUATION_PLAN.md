...
- Latest verified proof public merge commit: `c0cb613 fix: harden v1.12.33 dogfood contracts`
...
- Main CI run `26094452260`: passed pre-release checks, semantic-release, `publish-github-release-assets`, `publish-pypi`, and `publish-success-gate`.
...
- PyPI pinned public install: `uvx --refresh-package tensor-grep --from tensor-grep==1.12.34 tg --version` reports `tensor-grep 1.12.34`.
- GitHub release assets for `v1.12.34` include native CPU front doors, checksums, winget manifest, Homebrew formula, and publish instructions.
