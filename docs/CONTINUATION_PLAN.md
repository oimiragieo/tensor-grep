...
- Main CI run `25635877849`: passed through semantic-release, `publish-github-release-assets`, `publish-pypi`, and `publish-success-gate`
- CodeQL run `25636164492`: passed
- PyPI latest and pinned public install: `tensor-grep==1.9.7` resolves from PyPI.
...
- Managed native dogfood: direct `C:\Users\oimir\.tensor-grep\bin\tg.exe --version` reports `tg 1.9.7`; the bare `tg` command on this host is shadowed by unrelated Together CLI unless the non-destructive tensor-grep `tg.com` bridge in the same directory is current, because Machine PATH ordering is not writable.
