...
- Main CI run `25645819170`: passed pre-release checks, semantic-release, and `publish-github-release-assets`; PyPI publish was skipped after transient crates.io DNS broke the macOS wheel build, and `publish-success-gate` failed as designed
- CodeQL run `25646156907`: passed
- PyPI latest and pinned public install: `tensor-grep==1.9.9` resolves from PyPI.
...
- Managed native dogfood: direct `C:\Users\oimir\.tensor-grep\bin\tg.exe --version` reports `tg 1.9.9`; `tg update` advanced sidecar/native from `1.9.8` to `1.9.9`, and fresh `cmd` plus unprofiled `pwsh` report `tg 1.9.9`.
