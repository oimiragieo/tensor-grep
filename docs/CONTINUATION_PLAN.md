...
- Main CI run `25647256985`: passed pre-release checks, semantic-release, `validate-pypi-artifacts`, `publish-github-release-assets`, `publish-pypi`, and `publish-success-gate`.
- CodeQL run `25647635021`: passed on the release commit.
- PyPI latest and pinned public install: `tensor-grep==1.9.11` resolves from PyPI.
...
- Managed native dogfood: direct `C:\Users\oimir\.tensor-grep\bin\tg.exe --version` reports `tg 1.9.11` after `tg update`; pinned `uvx --from tensor-grep==1.9.11 tg --version` reports `tensor-grep 1.9.11`.
