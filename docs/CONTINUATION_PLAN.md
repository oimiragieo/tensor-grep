...
- Main CI run `25631800575`: passed through semantic-release, `publish-github-release-assets`, `publish-pypi`, and `publish-success-gate`
- CodeQL run `25632107771`: passed
- PyPI latest and pinned public install: `tensor-grep==1.9.6` resolves from PyPI.
...
- Managed native dogfood: direct `C:\Users\oimir\.tensor-grep\bin\tg.exe --version` reports `tg 1.9.6`; the bare `tg` command on this host was shadowed by unrelated Together CLI until local dogfood added a non-destructive `tg.com` bridge ahead of the foreign `.exe` because Machine PATH ordering was not writable.
