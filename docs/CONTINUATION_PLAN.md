...
- Main CI run `25800094003`: passed pre-release checks, semantic-release, `publish-github-release-assets`, `publish-pypi`, and `publish-success-gate`.
- CodeQL run `25800093062`: passed on the v1.10.9 release line.
- PyPI pinned public install: `uvx --refresh-package tensor-grep --from tensor-grep==1.10.9 tg --version` reports `tensor-grep 1.10.9`.
...
- Managed native dogfood: direct `C:\Users\oimir\.tensor-grep\bin\tg.exe --version` reports `tg 1.10.9` after `tg upgrade`; fresh `cmd` and unprofiled `pwsh` also report `tg 1.10.9`.
...
- Python subprocess launcher dogfood is separate from shell dogfood. `subprocess.run(["tg", ...])` can hit a foreign `.exe` route that `cmd`/PowerShell hide through `PATHEXT`, so readiness should check it explicitly and `tg doctor --json` should expose the route.
