...
- Latest verified proof public merge commit: `b0c7cf6 fix: harden v1.13.14 dogfood contracts`
...
- Main CI run `26386327552`: passed pre-release checks, semantic-release, `publish-github-release-assets`, `publish-pypi`, and `publish-success-gate`.
...
- PyPI pinned public install: `uvx --refresh-package tensor-grep --from tensor-grep==1.13.15 tg --version` reports `tensor-grep 1.13.15`.
- GitHub release assets for `v1.13.15` include native CPU front doors, checksums, winget manifest, Homebrew formula, and publish instructions.
