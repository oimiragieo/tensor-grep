...
- Main CI run `25637706454`: passed through semantic-release, `publish-github-release-assets`, `publish-pypi`, and `publish-success-gate`
- CodeQL run `25637706166`: passed
- PyPI latest and pinned public install: `tensor-grep==1.9.8` resolves from PyPI.
...
- Managed native dogfood: direct `C:\Users\oimir\.tensor-grep\bin\tg.exe --version` reports `tg 1.9.8`; `tg update` advanced sidecar/native from `1.9.7` to `1.9.8`, and the released `1.9.8` updater refreshed the tensor-grep `tg.com` bridge so fresh `cmd` and unprofiled `pwsh` report `tg 1.9.8`.
