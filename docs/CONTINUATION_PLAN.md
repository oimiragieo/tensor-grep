...
- Main CI run `25670325770`: passed pre-release checks, semantic-release, `publish-github-release-assets`, `publish-pypi`, and `publish-success-gate`.
- CodeQL run `25670325881`: passed on the v1.10.0 release line.
- PyPI pinned public install: `uvx --refresh --from tensor-grep==1.10.0 tg --version` reports `tensor-grep 1.10.0`.
...
- Managed native dogfood: direct `C:\Users\oimir\.tensor-grep\bin\tg.exe --version` reports `tg 1.10.0` after `tg upgrade`; copied `tg.com` bridge sidecar-backed command failure is the current follow-up blocker.
