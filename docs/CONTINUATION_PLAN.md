...
- Latest verified proof public merge commit: `b0e5c27 fix: harden v1.13.16 dogfood followups (#228)`
...
- Main CI run `26420296271`: passed pre-release checks, semantic-release, `publish-github-release-assets`, `publish-pypi`, and `publish-success-gate`.
...
- PyPI pinned public install: `uvx --refresh-package tensor-grep --from tensor-grep==1.13.17 tg --version` reports `tensor-grep 1.13.17`.
- GitHub release assets for `v1.13.17` include native CPU front doors, checksums, winget manifest, Homebrew formula, and publish instructions.
