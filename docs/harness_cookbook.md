...
- `"symbol_navigation": "parser-backed-refs-callers:csharp-go-java-javascript-php-python-rust-typescript+foundational-defs-imports-only:c-cpp"` -- honest about tiers: `tg refs`/`tg callers`/`tg blast-radius` are AST/tree-sitter-verified only for the first group; the second group has parser-backed defs/imports but refs/callers fall back to a regex-heuristic text match.
