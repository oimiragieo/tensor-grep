...
- `tg prepare PATH "task" --json` is the one-call edit-readiness surface for agents: primary target with confidence, a callers/blast-radius floor, detected validation commands, and an ask-before-editing flag in a single capsule — and when a `--deadline` binds it exits `2`, marks the result partial, and downgrades its own confidence score rather than presenting a truncated scan as complete. See [docs/tool_comparison.md](tool_comparison.md) for how this surface compares.
