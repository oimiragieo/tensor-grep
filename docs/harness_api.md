...
| `confidence` | `object` | Overall score and downgrade reasons, matching the `agent` capsule contract's top-level `confidence` shape (`overall`, `downgrade_reasons`). Computed via the same trust-check + confidence + ambiguity ladder `agent` uses (including the alternative-target-tie 0.74 cap) -- see the Agent Capsule JSON `confidence` row below and `docs/CONTRACTS.md`'s edit-planning contracts for exactly which agent-only snippet/call-site/LSP-evidence enrichments are (honestly) never applied here. |
| `ask_user_before_editing` | `object` | `required` plus reasons, matching the `agent` capsule contract's top-level `ask_user_before_editing` shape. As the hard auto-edit safety gate it matches `agent` on ambiguity: set when an alternative target ties the primary, the primary is an unrequested marker-helper, validation-command evidence is absent, confidence is below 0.75, the underlying repo scan was truncated, or a trust-check downgrade fired (query-language mismatch, unaligned validation evidence). |
...
| `path` | `string` | Absolute root path inventoried. |
| `scan_limit` | `object` | Optional bounded-scan metadata with `max_repo_files`, `scanned_files`, and `possibly_truncated`. Agent-facing CLI commands default to bounded broad scans. On `tg_search` (MCP contract `1.5.0`+) a TRUNCATED scan additionally carries `truncation_cause` (`"scan_limit"` \| `"unreadable_path"` \| `"unknown"`), `budget_remediable` (`bool`), and — when non-zero — `unreadable_path_count`. Read `budget_remediable` before advising a retry: it is `false` for `"unreadable_path"` because no `max_repo_files` value makes a permission-denied directory readable, and `false` for `"unknown"` because an unrecognised cause must never be answered with "raise the limit". These three keys are ABSENT on a complete scan, so a complete payload is byte-identical to contract `1.4.0`. Note `possibly_truncated: true` can still appear WITHOUT `truncation_cause` (the per-file cap and the native-walk deadline set it independently of the scanner) — do not treat "truncated implies a cause is present" as an invariant. |
| `incomplete_reason_class` | `string` | Optional, top level (MCP contract `1.6.0`+). The CLOSED vocabulary (`"unreadable_path"` \| `"scan_limit"` \| `"deadline"` \| `"timeout"`) telling you WHY a partial result is partial, so you never string-sniff `incomplete_reason`. Emitted only when the cause is classifiable: both native-walk deadline paths emit `"deadline"`, a broad-scan refusal emits `"scan_limit"`. **ABSENT means "cause not classifiable", NEVER "scan complete"** — completeness is carried by `result_incomplete` alone, and an AST-backend failure deliberately sets `result_incomplete` with no class rather than mislabelling a backend fault as `"unreadable_path"`. Distinct from the `scan_limit.truncation_cause` vocabulary above, which is NOT the same set and must not be conflated. Absent on a payload with no classified cause, so that body stays byte-identical to contract `1.5.0`. |
...
| `output_limit` | `object` | Present when display budgets are active. Raw CLI `blast-radius` defaults to a 25-caller / 25-file budget; pass `--max-callers` or `--max-files` to raise or lower it. Symbol commands also use `--max-tests`. Read `callers_truncated`, `files_truncated`, `tests_truncated`, and `import_consumers_truncated` plus each category's exact total/returned/omitted counts. These are display omissions: an output-only cap keeps `result_incomplete: false`, exits 0, and emits an `OUTPUT LIMITED` advisory on stdout; the subset is not exhaustive. |
