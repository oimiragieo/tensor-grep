...
Pin existing non-WSL argv byte-for-byte before changing routing. Add exact public-route tests for bootstrap `search`, full Typer search spawning native, and direct native `search`. The typed census is positional roots, pattern-file `-f/--file`, and `--ignore-file`; cover every intended separate, attached, equals, and valid bundled short spelling independently. For a spelling the current router or Clap grammar cannot reach, first write a public registration RED for that exact spelling, implement only enough routing/grammar registration to make it green, and freeze that green test. Only then write the spelling's semantic translation RED through the now-reachable production route; that RED must observe the raw POSIX operand reaching the Windows boundary, not a missing module, seam, parser rejection, or sentinel. Already-reachable spellings proceed directly to the semantic RED. Add opaque controls whose pattern, glob, filter, and other flag values look like `/mnt/...`; these stay byte-identical. Add relative, `.`, `..`, `-`, and already-Windows controls. Execute every node independently and preserve its failure text.
...
Before Step 2 changes the bridge, write every bridge/security behavior test and run each independently RED: poisoned `SystemRoot`; executable reparse/substitution/signature/identity changes; input, unique-path, child, aggregate, stdout, and stderr cap boundaries; duplicate caching; nonzero child; invalid UTF-8; empty/multiline/relative/drive-relative/root-relative/ADS/device/NT/oversize/control-character output; descendant-held pipes; Job termination; bounded reap; exact evidence/reason/error mappings; and zero read/child on every failure. Use existing monkeypatch/process boundaries where they can express the behavior. If a required adapter or symbol is absent, first record one import/registration RED, add only a behaviorless shell, then run the independent semantic REDs before implementing that behavior. A configurable automount fixture proves no `/mnt` assumption.

**Step 2: implement bounded provenance and translation**

Implement Program 0A's bounded reserved-entry normalization. The shim overwrites and exports both values, preserves unrelated `WSLENV` entries byte-for-byte/in-order, appends exactly two canonical `/w` entries, and executes original `"$@"`. It does not run `eval`, `sh -c`, `cmd /c`, PowerShell interpolation, or any argv-rewrite loop. Native/Python accept both variables absent as legacy only; partial/invalid provenance is `invalid_provenance`/exit 2.

Implement `PathDomainContractV1`, `PathDomainEvidenceV1`, and the shared fixture corpus behind injectable Windows process/clock/handle/signature adapters. Resolve System32 with `GetSystemDirectoryW`; require exact final path, regular non-reparse identity, Microsoft `WinVerifyTrust`, and stable volume/file ID after child creation. Assign the suspended child to a kill-on-close Job Object before resume, drain both pipes through cap+1, kill descendants on every cap/timeout/decode/signature/identity failure, and boundedly reap. Pin the exact drive-absolute/ordinary-UNC grammar and device/NT/ADS/reserved-name rejections from the design. Logs/errors disclose only domains and exhaustive reason code, never raw usernames, distro names, paths, or child stderr.

Extend the grammar-derived field registry: `_search_path_args_raw` remains the positional-root classifier, while explicit option-operand parsing owns `-f/--file` and `--ignore-file` in all accepted spellings. Do not infer paths from slash-shaped arbitrary tokens. Python owns the complete argv it sends to native; direct native owns its complete argv. A consumed child marker is valid only when no absolute POSIX typed field remains. Preserve result paths in Windows form. Tensor-grep structured output uses the exact nested evidence object and failure fields; raw-ripgrep compatibility JSON/NDJSON plus ordinary successful text stay byte-identical.

**Step 3: adversarial and mutation proof**

Rerun all Step 1 semantic nodes green, including executable distro UTF-8 at 255/256/257 bytes plus empty/control, pure-parser embedded-NUL rejection, input at 16 KiB boundaries, 256/257 unique paths, unset/exactly-empty/malformed/cap-boundary `WSLENV`, every process/output/failure arm, one sanitized stderr snapshot, and no exec/read/child on failure. For every spelling, retain both its registration test (when one was needed) and its later semantic test. As post-green sensitivity proof, temporarily remove each positional/pattern-file/ignore-file bootstrap/full/native semantic mapping or one bridge guard from production while leaving its public semantic test unchanged, require the expected raw-operand/evidence/no-read/no-child/fail-closed assertion to fail, restore, and rerun green. Post-green mutations supplement but never replace the recorded Step 1 REDs.
...
Push the first genuine RED commit, open a draft PR, immediately move #89 to `IN_FLIGHT` with its real PR number and ordered implementation list, then build TDD-first. Require independent specification and adversarial security `SHIP`, focused real-venv tests, Ruff/preview-format/mypy, and non-skipped Windows/native CI whose receipt lists every fake-bridge node ID. Merge only after the newest main run is completed and any preceding release is served by PyPI. Dogfood from the exact tagged public `install.ps1`/release asset in a clean WSL location: record URL/SHA-256, generated shim digest, selected binary path/version/hash, mixed-flag and cap-boundary `WSLENV`, distro cap boundaries, match exit 0, no-match exit 1, `-f/--file` and `--ignore-file` in every spelling family, no-read failure, and translation failure exit 2/no raw child. Also prove managed upgrade regenerates the fixed shim. Exclude source-tree/global shims. #89 remains `IN_FLIGHT` until Task 2C and the separate closure PR finish.
...
- #89 search match exit 0, no-match exit 1, positional roots plus `-f/--file` and `--ignore-file` spelling families, exact `PathDomainEvidenceV1`, and failure exit 2/no read/no raw child through bootstrap/full/direct-native ownership arms;
