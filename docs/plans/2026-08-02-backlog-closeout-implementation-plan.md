...
**Convergence record:** round 15 reached three-seat `SHIP`, but exact-hash rounds 16–18 found new
architecture/security/TDD blockers before implementation. After amendment, all three seats returned
`SHIP` on the same pair. The design status and this paragraph are the sole subsequent changes; Task 2
remains frozen until all three seats confirm the final status-stamped hashes.
...
Pin the shared service/CLI workspace schema version 1 exactly: `{version:1, schema_version:1, workspace_prepare_version:1, routing_backend:"prepare-service-v1", routing_reason:"explicit-roots"|"invalid-input", anchor:str|null, query:str|null, roots:list[WorkspaceRootV1], root_count:int, completed_root_count:int, omitted_roots:list[OmittedRootV1], result_incomplete:bool, incomplete_reasons:list[WorkspaceIncompleteReason], error:WorkspaceErrorV1|null}`. `WorkspaceRootV1` is exactly `{root:str, status:"complete"|"partial"|"error", payload:dict[str,JSONValue]|null, error:WorkspaceErrorV1|null, incomplete_reason:null|"deadline"|"prepare_incomplete"|"prepare_error"|"output_limit"}` with the design's status/nullability rules. `WorkspaceErrorV1={code:"invalid_input"|"prepare_error", message:nonempty-str<=1024-UTF8-bytes}`; `OmittedRootV1={root:str, reason:"deadline"}`; `WorkspaceIncompleteReason` is the ordered subset `invalid_input,deadline,prepare_incomplete,prepare_error,output_limit`. Require disjoint canonically sorted roots/omissions, `root_count == len(roots)+len(omitted_roots)` for valid canonical input, `completed_root_count == count(status=="complete")`, and exact `result_incomplete` equivalence to an error, omission, noncomplete root, or output limiting. Success and dispatched partial results set top-level `error=null`. Zero/duplicate/nested/out-of-anchor/>8-root input returns the same full key set with canonical `anchor` or null, valid-string `query` or null, `roots=[]`, `omitted_roots=[]`, counts zero, `routing_reason="invalid-input"`, `result_incomplete=true`, `incomplete_reasons=["invalid_input"]`, and `error={"code":"invalid_input","message":...}`, then exits 2. Output-cap rebuilding preserves root/count/error identity, nulls payloads, and adds root/top-level `output_limit`. Any partial/omitted/failed root after dispatch emits JSON then exits 2. One root still returns the workspace schema; legacy `tg prepare` remains byte-identical. Exact type/nullability/invariant/vocabulary tests and same-fixture value-equality tests compare service, Typer CLI, and compiled native CLI for success, partial, invalid, omitted, and output-limit arms.
...
- canonical tracker parser/version controls, each Task 2 semantic reconciliation node, every Tasks 3–13 owning row's real-PR `READY` → `IN_FLIGHT` transition, and the separate post-merge `SHIPPED` closure transition;
