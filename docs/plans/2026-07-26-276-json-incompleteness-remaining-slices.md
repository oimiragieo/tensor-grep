...
- [ ] ~~**Step 1:** Rename inside #795 before merge — `incomplete_event_count` or `walk_error_count`. Two-line diff (`:846` struct, `:2438` emit) plus #795's own CONTRACTS bullet.~~ **DO NOT DO THIS** — #795 merged and the field shipped in v1.99.5; see the 2026-07-27 block at the top of this task. **Replacement Step 1:** extend the `incomplete_paths_count` bullet in `docs/CONTRACTS.md` to state it counts walk-error EVENTS, that duplicate/overlapping roots make it exceed the distinct-path count (measured: same root twice → 2, one distinct path), and that this matches rg. Still keep it DISTINGUISHABLE from MCP's `unreadable_path_count` (`CONTRACTS.md:176`), which is a genuine per-path count (`directory_scanner.py:227`, once per failed `os.scandir`) — converging them repeats the task 293 mistake.
