...
## 2026-09-08 Research Addenda

### Competitive feature research (Exa, read-only)
...
- **F1 (bundled AST security-detector pack):** `tg scan --ruleset` is a generic rule engine; no
  ...
  gap explicitly named in third-party competitor comparisons. Seat: Developer — Hard, Codex Sol
  (well-specified pattern authoring). Depends on: the existing `scan --ruleset` engine +
...
- **F2 (git-SHA-pinned agent decision memory):** no competitor surveyed (including tensor-grep)
  ...
  of the existing session/checkpoint subsystem rather than new infrastructure. Seat: Developer —
  Extreme, Codex Astra or Fable 5.1 xhigh (new persistent-state subsystem; needs a
  design-authorization pass per `tensor-grep-design-authorization-ladder` before any code).
  Depends on: `session_store.py`, `checkpoint_store.py`.
- **F3 (published reproducible comparative benchmark):** competitors increasingly publish an
  ...
  `tensor-grep-benchmark-and-proof-toolkit`. Depends on: existing `scripts/dogfood/` + benchmark
  scripts, positioned as a new public artifact rather than new product code.
...
### P13-EXT2: baseline-dev-architecture follow-on (read-only audit)
