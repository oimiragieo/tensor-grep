...
- **Local test gate:** `uv run --no-sync pytest <narrow suite> -q`, then `uv run --no-sync pytest tests/unit tests/integration -q --maxfail=0` before push. **Do NOT run a bare `pytest -q --maxfail=0`:** `testpaths = ["tests"]` (`pyproject.toml:35`) has no E2E exclusion, so a bare run COLLECTS `tests/e2e/test_routing_parity.py` (verified: 53 tests), which resolves cargo via `_resolve_cargo_exe` (`:117-119`) and self-compiles Rust -- violating the shared-server rule two lines above. `-x` in the default addopts can mask this: a `--collect-only` check aborts on the first unrelated error and returns a FALSE ZERO for the parity module. `uv run --no-sync mypy src/tensor_grep` (strict mode — annotate everything).
