...
release_docs_current_tag: v1.12.8

- Latest tagged version: `v1.12.8`
- Latest complete PyPI version: `v1.12.8`
...
- Latest fix commit: `f848748 fix: route cold rg-shaped searches to rg (#137)`
- Latest feature commit: `a518cc6 feat: add agent success harness`
- GitHub release: <https://github.com/oimiragieo/tensor-grep/releases/tag/v1.12.8>
...
- Main CI run `25927793134`: passed the pre-release matrix, semantic-release, `publish-github-release-assets`, `publish-pypi`, and `publish-success-gate`
...
- PyPI pinned install: `uvx --refresh-package tensor-grep --from tensor-grep==1.12.8 tg --version` reports `tensor-grep 1.12.8`
- GitHub release assets: `v1.12.8` has uploaded native CPU front doors for Windows/Linux/macOS, checksums, winget manifest, Homebrew formula, and publish instructions
- Public `v1.12.8` dogfood verified PyPI, `uvx`, release assets, public launcher resolution, accepted search/native-front-door CLI drift fixes, root cold rg-shaped routing, `tg run --pattern`, bounded map/context output, `tg new --base-dir`, edit-plan budget flags, and explicit JSON/NDJSON schema positioning. Public managed GPU is not promotion-ready: explicit GPU requests fall back to `NativeCpuBackend` or unsupported rows unless a CUDA-feature native build proves `NativeGpuBackend` with `sidecar_used = false`.
- In-progress `v1.12.8` dogfood follow-up branch `fix/v1128-dogfood-contracts` addresses the latest enterprise blockers: root `tg --format rg ...` and `tg --format rg --json ...` now route before the positional parser; explicit `tg search --format rg --json` streams ripgrep JSON Lines while default `tg search --json` stays tensor-grep aggregate JSON; docs/help now call search a validated common rg-compatible subset rather than a full ripgrep replacement; repo-map context excludes `artifacts`, coverage, tox/nox/cache roots by default; edit-plan file summaries honor symbol/source budgets; and agent capsules require confirmation when an unrequested marker-helper primary ties implementation alternatives. Local validation is green: `uv run ruff check .`, `uv run ruff format --check --preview .`, `uv run mypy src/tensor_grep`, `C:\Users\oimir\.cargo\bin\cargo.exe fmt --manifest-path rust_core\Cargo.toml --check`, `uv run pytest -q` (`2124 passed, 16 skipped`), full `cargo test --manifest-path rust_core\Cargo.toml`, `python scripts\agent_readiness.py --no-wsl-probe --output artifacts\agent_readiness_v1128_fix_contracts.json`, `uv run tg dogfood --output artifacts\dogfood_readiness_v1128_fix_contracts.json`, and `git diff --check` all pass.
...
Current release branch is publication-complete for `v1.12.8`: PR #137 `fix: route cold rg-shaped searches to rg` was squash-merged as `f848748`, release commit `cfc49b0 chore(release): v1.12.8 [skip ci]` exists, main CI run `25927793134` passed tests/assets, GitHub asset upload, PyPI publish, and `publish-success-gate`, and CodeQL run `25927792509` passed. Public dogfood verified PyPI, release assets, `uvx`, launcher resolution, accepted native/dev CLI flag alignment, root cold rg-shaped routing, `tg run --pattern`, bounded map/context output, `tg new --base-dir`, edit-plan budget flags, and explicit JSON/NDJSON schema positioning. The `v1.10.10` Windows subprocess launcher repair remains the historical public launcher repair baseline; keep foreign launcher handling opt-in and auditable rather than deleting unrelated tools.
...
Active post-`v1.12.8` implementation scope:
