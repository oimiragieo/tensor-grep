...
- Latest fix commit: `23e5f52 fix: harden GPU gates and launcher diagnostics`
- Latest feature commit: `95bfd81 feat: add actionable agent context capsule`
- GitHub release: <https://github.com/oimiragieo/tensor-grep/releases/tag/v1.9.5>
...
- PyPI latest and pinned install: `tensor-grep==1.9.5` resolves from PyPI
- GitHub release assets: `v1.9.5` has uploaded native CPU front doors for Windows/Linux/macOS, checksums, winget manifest, Homebrew formula, and publish instructions
...
- Closed docs/version governance and validation placeholder gap: `v1.9.4` aligns public docs and governance tests with the current project release tag, and `tg run --apply --verify --lint-cmd 'python -m py_compile "$file"'` substitutes `$file` / `{file}` before validation so paths with spaces can be quoted safely. Post-`v1.9.5` dogfood extends that contract for directory rewrites: placeholder validation commands run once per edited file, while placeholder-free validation commands still run once in the original target working directory.
...
- Public `v1.9.5` dogfood: direct `C:\Users\oimir\.tensor-grep\bin\tg.exe --version` reports `tg 1.9.5`; PyPI latest and pinned public install resolve `tensor-grep==1.9.5`; an unrelated Together CLI `tg.exe` originally shadowed bare `tg` in fresh Windows shells, and local dogfood was repaired non-destructively by placing a tensor-grep `tg.com` bridge ahead of that foreign `tg.exe` because Machine PATH ordering was not writable.
...
- Keep GPU experimental until native streaming correctness and speed are proven on the 1GB/5GB gates. Local post-`v1.9.5` work proves both RTX 4070 and RTX 5070 can execute native and sidecar GPU smoke paths when the sidecar uses `cu128`, but the measured GPU rows are still slower than `rg` and `tg_cpu`; explicit `--gpu-device-ids` routing must stay scoped to selected devices and sidecar-routed rows must not count as native CUDA scale-gate timings.
...
- The `v1.9.4` release fixes stale docs-governance expectations and substitutes `$file` / `{file}` validation placeholders before executing validation commands. The post-`v1.9.5` blocker fix makes directory-level rewrite validation expand file placeholders per edited file instead of compiling the root directory.
