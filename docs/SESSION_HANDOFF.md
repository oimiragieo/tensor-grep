...
release_docs_current_tag: v1.12.10

- Latest tagged version: `v1.12.10`
- Latest complete PyPI version: `v1.12.10`
...
- Latest fix commit: `21627d2 fix: harden v1.12.8 dogfood contracts`
- Latest feature commit: `a518cc6 feat: add agent success harness`
- GitHub release: <https://github.com/oimiragieo/tensor-grep/releases/tag/v1.12.10>
...
- Main CI run `25941933937`: passed the pre-release matrix, semantic-release, `publish-github-release-assets`, `publish-pypi`, and `publish-success-gate`
...
- PyPI pinned install: `uvx --refresh-package tensor-grep --from tensor-grep==1.12.10 tg --version` reports `tensor-grep 1.12.10`
- GitHub release assets: `v1.12.10` has uploaded native CPU front doors for Windows/Linux/macOS, checksums, winget manifest, Homebrew formula, and publish instructions
- Public `v1.12.9` dogfood verified PyPI, `uvx`, release assets, public launcher resolution, accepted search/native-front-door CLI drift fixes, root cold rg-shaped routing, `tg run --pattern`, bounded map/context output, `tg new --base-dir`, edit-plan budget flags, explicit rg JSON Lines routing via `--format rg --json`, and explicit JSON/NDJSON schema positioning. Public managed GPU is not promotion-ready: explicit GPU requests fall back to `NativeCpuBackend` or unsupported rows unless a CUDA-feature native build proves `NativeGpuBackend` with `sidecar_used = false`.
- In-progress post-`v1.12.10` dogfood follow-up branch `fix/v1129-rg-flag-contracts` addresses the next rg flag-contract slice: Python rg passthrough forwards advertised ignore/type/output controls, `tg search` accepts `--maxdepth` as the ripgrep alias for `--max-depth`, and public native/root rg-shaped forwarding accepts `--sort-files`. `--print0` is intentionally not added because current ripgrep 15.1 rejects it; the compatible NUL path-list flag is `-0` / `--null`.
...
Current release branch is publication-complete for `v1.12.9`: PR #138 `fix: harden v1.12.8 dogfood contracts` was squash-merged as `21627d2`, release commit `b15f71a chore(release): v1.12.9 [skip ci]` exists, main CI run `25941933937` passed tests/assets, GitHub asset upload, PyPI publish, and `publish-success-gate`, and CodeQL run `25941933444` passed. Public dogfood verified PyPI, release assets, `uvx`, launcher resolution, accepted native/dev CLI flag alignment, root cold rg-shaped routing, `tg run --pattern`, bounded map/context output, `tg new --base-dir`, edit-plan budget flags, explicit rg JSON Lines routing, and explicit JSON/NDJSON schema positioning. The `v1.10.10` Windows subprocess launcher repair remains the historical public launcher repair baseline; keep foreign launcher handling opt-in and auditable rather than deleting unrelated tools.
...
Active post-`v1.12.10` implementation scope:
