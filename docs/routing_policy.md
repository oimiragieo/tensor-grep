...
- **stdin is not readable** (not a pipe or file redirect; a TTY or `/dev/null` is fine). A readable stdin means two candidate input sources, and tg's request resolution already branches on it. Real `rg` renders the same bytes either way for an explicit single path, so this is not a rendering divergence -- it is an unverified region of the tg/rg argv contract, refused because over-refusal is free. The condition is `grep_cli::is_readable_stdin()`, deliberately not `!stdin.is_terminal()`: the latter would refuse in every non-interactive context, including CI and agents, and would delete the feature it is meant to protect.
