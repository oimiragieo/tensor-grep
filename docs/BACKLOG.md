...
|---|---|---|
| **AGT-01** | IN PROGRESS / mixed: baseline `wrong_confident_primary` reporting shipped in `dd3c594`; PR #1159 (`bb635941`) shipped a fail-closed six-field outcome join, and PR #1160 (`cc833b06`, released in v1.121.0) wired verified outcomes into scorecards without changing ranking scores. Holdout manifest curation remains blocked on real repository fixtures plus independently reviewed known-good patches; do not flatten the shipped measurement path and blocked holdout corpus into SHIPPED. | [Task 01](plans/2026-09-07-agentic-quality-simplification.md#task-01) |
| **AGT-02** | SHIPPED (`3e67c45`, PR #1136, merged to main, full CI green including all `test-python`/`test-rust-core`/GPU/benchmark matrices): race fix (`611ca8d`) plus versioned `decision_generation`/`current_generation`/`decision_freshness` (current/historical/unknown) metadata on `last_prepare`, changed-content-retains-history, and legacy-payload-resolves-unknown behavior, covered by new tests in `tests/unit/test_session_resume.py`. | [Task 02](plans/2026-09-07-agentic-quality-simplification.md#task-02) |
| **AGT-03** | IN PROGRESS (`e163de4`, PR #1138 merged, full CI green): `tg prepare --include-next-action` no longer hardcodes `uv run pytest -q` for every repository; `on_success.argv` is now selected only from an exact-match allowlist of the capsule's own detected `validation_commands`, falling back to `on_success.unavailable` when nothing matches. `prepare_protocol.py` now exists with a frozen `ValidationAdvice` record separating detected-recipe identity from execution authority; `prepare_service.py` builds/consumes it internally, byte-identical `on_success` output. Centralized `stop_if` (non-current-decision) and versioned canonical session serialization remain open. | [Task 03](plans/2026-09-07-agentic-quality-simplification.md#task-03) |
| **AGT-04** | IN PROGRESS (`9377ea4`): `_walk_tracked_files_bounded` prunes dependency-tree dirs (node_modules/.venv/target/dist/etc) via `os.walk` topdown pruning before descending, replacing an unbounded `rglob`; adds finite file-count/per-file-byte/aggregate-byte budgets that report explicit `{"status": "incomplete", ...}` instead of silently truncating; `verify_edit_ticket` now fails closed (`population_incomplete`) on a ticket built from an incomplete population; tracked dotfiles still hashed. **Design doc drafted, NOT REVIEWED**: `docs/design/2026-09-10-agt04-symlink-junction-confinement.md` -- `os.walk` descends into a Windows junction under root; proposes reparse-aware pruning + `lstat` + git-tracked identity as primary source. Security review required -- CEO-gate. | [Task 04](plans/2026-09-07-agentic-quality-simplification.md#task-04) |
| **AGT-05** | IN PROGRESS (`bb1aa70`): `_build_confidence_calibration()` adds a confidence-calibration REPORT to the agent-workflow benchmark summary -- fixed-width bins over `confidence_overall` (count + observed `hit_at_1` correctness per bin) plus a selective-accuracy-vs-answer-coverage curve; `confidence_kind` stays `"heuristic"`, zero scored tasks reports `insufficient_evidence=True` rather than a silent 100%. Two real defects from a codex_luna audit (NaN/inf/out-of-range confidence crashing bucketing; float-division bin-boundary misclassification) were fixed with regression tests before commit; AUDIT_CLEAR round 2. Pure reporting scaffold -- no scorer/ranking code touched. DEVELOPMENT-data paraphrases + holdout-safe hardcases, feature tracing for wrong-primary causes, one-hypothesis-at-a-time scorer ablation with a no-change control, and order-pin re-verification remain open. | [Task 05](plans/2026-09-07-agentic-quality-simplification.md#task-05) |
| **P9** | IN PROGRESS (`eb6dec7`, PR #1140 merged, full CI green): `docs/design/2026-09-07-cache-ownership.md` records the component-ownership map and formalizes dormant `semantic_index.py` as **RETAIN, library-only, not adopted**. Zero-redundant-warm-map-builds gap now closed: `session_prepare` reuses the session's own `repo_map` when its `current_generation` stamp matches a fresh `_snapshot_generation` recomputation, skipping `build_repo_map` entirely (spy-control-proven zero calls on the warm path; a drifted stamp correctly falls back to a cold rebuild). Lock-hold-time measurement and generation-safe BM25 publication remain open. | [Task 06](plans/2026-09-07-agentic-quality-simplification.md#task-06) |
...
| **AGT-06** | Direct-main `390c39f`: `io/root_probe.iter_top_level_vendored_dirs` factors the shared one-level-iterdir/OSError-swallow/case-insensitive scan out of bootstrap's short-circuit and main.py's sorted-diagnostics adapters; each keeps its own aggregation, return shape, and argv/SearchConfig handling. Frozen fixture-table parity test locked pre-refactor baseline green, then re-confirmed green post-refactor (174/174 + 288/288 wider sweep); Codex Luna (read-only) AUDIT_CLEAR. All Task 08 checkboxes covered. | [Task 08](plans/2026-09-07-agentic-quality-simplification.md#task-08) |
| **AGT-07** | IN PROGRESS (`756769f`, PR #1137 merged, full CI green): `core/completeness.CompletenessEvidence` -- immutable, enum-typed (`ScanState`/`Cause`/`RetryKind`) record over the existing `unified_incomplete_envelope()` dict projection; `from_legacy_envelope()`/`to_legacy_dict()` round-trip byte-for-byte, proven against the frozen truth table plus 2 mutation controls. `mcp_server.py`'s `_inject_mcp_contract_fields` now migrated (constructs the typed record internally via `core.completeness.project()`, projects back out with `to_legacy_dict()`; byte-identical, per-row truth-table equivalence tests + 2 consumer-level mutation controls). `main.py`'s own ~74 `result_incomplete`/`budget_remediable` references, woven through its `Result` dataclass across many call sites, remain the higher-risk half of Task 09 not yet migrated. | [Task 09](plans/2026-09-07-agentic-quality-simplification.md#task-09) |
| **MCP-SURFACE** | IN PROGRESS (`c9e25f1`), Task 10 checkbox 1 of 8: standalone opt-in `mcp_followup_ref.mint_followup_ref()`/`resolve_followup_ref()` primitive -- HMAC-signed reference binding an omitted-content range to root, a root-confined path (no traversal/symlink escape), a SHA-256 content-hash snapshot, exact request params, and a finite TTL with exclusive expiry. Registers no MCP tool, not called from `mcp_server.py`'s default path (`_TG_MCP_SERVER_CONTRACT_VERSION` unchanged at the time of that entry, and gated behind Task 2C per the then-current MCP-SURFACE ladder -- **that fence is STALE as of 2026-09-12**: the live contract is already **1.8.0**, bumped by `5b8c85e` (`feat(mcp): standardize incompleteness envelope across MCP tools (P3)`, PR #1135), not by Task 2C, so the "must not bump from a nonexistent base" rationale no longer holds and this row needs re-triage on its own merits rather than a Task-2C wait (see the MCP-SURFACE row in docs/TASK_BOARD.md)). 4 rounds of Codex Luna audit fixed forgeable snapshot identity, unconfined traversal, and NaN/inf/bool schema-smuggling; a documented TOCTOU limitation remains by design (descriptor-only, not a byte-server). Response profiles, byte caps, structuredContent/outputSchema negotiation, and tools/list cursor semantics remain open; see `docs/design/2026-09-07-mcp-followup-ref-prototype.md`. | [Task 10](plans/2026-09-07-agentic-quality-simplification.md#task-10) |
| **AGT-08** | IN PROGRESS (`6e80d4f`, PR #1139 merged, full CI green): `core/retrieval_fusion.reciprocal_rank_fusion_explained()` delegates fused order to the existing `reciprocal_rank_fusion` (fuzz-proven byte-identical across 25 random ranking shapes, both combine modes) and returns a real per-leg-term `FusionExplanation` per chunk. `tests/unit/test_find_why_ranked.py` now pins that `tg find --why-ranked` never alters returned match order (Task 11's first checkbox), the frozen baseline required before threading `FusionExplanation` through find results. Recording contributions at their actual computation points, the additive explanation record threaded through find results, fixtures for lexical/dense/tie/dense-unavailable, and wiring into `tg search`'s CLI/MCP output remain open -- see Task 11's remaining 7 checkboxes. | [Task 11](plans/2026-09-07-agentic-quality-simplification.md#task-11) |
| **P12** | IN PROGRESS (`1f6f4b0`): `core/retrieval_dense.DenseStageTimings` + `DenseIndex.query_with_timings()` separately time corpus-encode (recorded at construction), query-encode, score, and sort; `query()` delegates to the timed variant so ranking/tie-break order is provably unchanged. `benchmarks/profile_dense_stages.py` is the min-of-N measurement harness (noise-floor discipline), synthetic corpus (no fetch dependency to run), skips cleanly when `model2vec`/the fetched model are absent -- verified on this box, which lacks the extra. Fake-encoder tests (18/18) cover the instrument without a real model. Model/corpus reuse-by-identity, bounded top-k selection vs full sort, and the actual float16/int8/ONNX comparison arm remain open. | [Task 12](plans/2026-09-07-agentic-quality-simplification.md#task-12) |
...
| **P10** | IN PROGRESS (`0f5a1f4`): `_rollback_risk_from_blast_radius`'s `test_count`/`coverage_factor` renamed to `associated_test_count`/`test_association_factor` with a docstring disclosing the count is a static import-scan, not measured runtime coverage; numeric risk-score output unchanged (parity-proven). Trusted observed execution at a pinned revision, plus bounded process/output and replay controls, remains open -- `evidence_receipt.py`/`evidence_signing.py` already aggregate already-computed revision identity but do not themselves execute/bound a test command. | [Task 15](plans/2026-09-07-agentic-quality-simplification.md#task-15) |
...
- **[x] S1/S5 — Fail-Closed Edit Tickets & Verify-Edit Service (`EditReadyTicketV1`)**
  ...
  - **CRITICAL follow-up round 1 (2026-09-06):** `verify_edit_ticket` only checked that the caller-supplied `modified_files` list was a subset of `allowed_files` — it never recomputed `pre_edit_fingerprints`/`working_tree_fingerprint` against actual tree state. The service was confirmed unwired (`grep -rln verify_edit_ticket src/` = only the file itself) so no live blast radius, but the core contract was broken on `main`. Fixed `86caffc`. TDD: 2 RED tests, 6/6 green after.
  - **CRITICAL follow-up round 2 (2026-09-06, delta-verification found round 1 incomplete):** the whole-tree walk excluded EVERY dot-prefixed path (`part.startswith(".")`), not just VCS/cache internals — so a legitimate tracked dotfile (`.github/workflows/*`, `.gitignore`) could be tampered with outside `allowed_files` and escape both fingerprint snapshots entirely, still returning PASS. Fixed `178a0aa`: exclude only the literal `.git` directory and `__pycache__`. TDD: 1 RED test simulating undeclared CI-workflow tampering, 7/7 green after.
  - **HIGH, NOT YET FIXED (round 2, deliberately deferred — real feature work, not a same-turn fix):** (1) no ticket-identity/atomicity validation — `verify_edit_ticket` never binds `repo_root` to `ticket.repo_root`, never validates ticket version/age/ID, so a caller holding a deserialized ticket could redefine both its allowlist and baseline; (2) the filesystem snapshot is non-atomic (enumerate-then-hash with no lock or second-pass census), so a file changed between enumeration and hashing can escape verification. Both need real design work (ticket signing/binding scheme, snapshot atomicity strategy) before implementing, not a quick patch.
...
- **[x] P3 — Standardize MCP Incompleteness Protocol Envelope**
  ...
  - **Receipt:** Merged PR #1135, squash SHA `5b8c85e`, CI was fully green pre-merge (40 pass, 0 fail, 0 pending). **Released as `v1.118.0` — FULLY AND FINALLY VERIFIED SHIPPED.** Every stage confirmed independently, not inferred from "CI is green": `Semantic Release` succeeded (tag + GitHub release published 2026-09-06 08:40:30 UTC); `publish-github-release-assets`/`publish-pypi` succeeded and PyPI holds all 4 expected artifact filenames (3 wheels + sdist, checked directly against `releases["1.118.0"]`, not `info.version` alone); `publish-success-gate` succeeded (the pipeline's own independent GH-assets + PyPI-parity re-check); `release-tag-smoke` succeeded (re-checked-out the actual `v1.118.0` tag and re-ran `agent_readiness.py` against it — the gate this repo has specifically seen mask a real regression for 4 releases in the past when skipped). Whole-run conclusion: `success`. Root cause of an earlier ~50min scheduling delay (CI prerequisites passing to `Semantic Release` actually starting) stayed undiagnosed — most likely GitHub-side runner-queue contention, no repo-level blocking cause found on inspection, and it self-resolved without intervention. Worktree + branch cleaned up. **10 real defects found and fixed across 3 independent Codex Sol audit rounds** before merge, every one with a RED-test-first reproduction, not a self-report:
    ...
    - Plus 1 real CI defect (not an audit finding): the round-3 fixes' own line additions drifted a pinned handler-disposition citation (`stdin_reader`) out of its span — same drift class as an earlier fix this session, fixed `8e1ab06`.
    - **Pattern worth naming explicitly:** each of the first 2 rounds looked complete until the next independent check found something real — see [[feedback-one-audit-round-is-not-enough-for-security-shaped-fixes-2026-09-06]]. Round 3 followed the same shape. **CI green is necessary but not sufficient here — not unilaterally declaring merge-ready.** Ready for either an operator go-ahead to merge, or a 4th independent audit check, whichever the operator prefers. Verified so far: 17 envelope tests + 39 search tests + 61 contract-fix tests + 74 session tests + 28 ratchet tests + 11 handler-disposition tests, all passing, each fix independently re-executed by the orchestrator (not trusted from a builder's report).
    - HIGH, not CRITICAL, genuinely deferred (real design work, not a same-turn fix): no ticket-identity/atomicity validation in the RELATED S1 edit-ticket service (see S1/S5 below).
  - **Do not merge until at minimum the two remaining CRITICALs above are fixed and independently re-verified** — this item has now needed 2 full audit-and-fix rounds; treat any future "done" claim on it with the same skepticism.

- **[ ] AUDIT-FOLLOWUP — Unverified HIGH findings from 2026-09-06 Codex Sol session audit**
  - **Objective:** Triage the remaining HIGH-severity findings from `docs/audits/codex-specs/2026-09-06-session-fanout-audit.md` that were NOT independently re-verified this session (unlike the 3 CRITICALs, which were confirmed real and fixed). Flagging honestly rather than silently dropping — Codex could not run tests/scripts under its read-only sandbox, so these are static-read claims, not test-confirmed.
  - **On already-merged `main` code:**
    - S3 — **CONFIRMED REAL, FIXED (`3829775`).** Direct read confirmed `include_next_action=True` was passed ONLY from `tests/unit/test_prepare_next_action.py`, never from `main.py`'s real `tg prepare` command — the entire feature was unreachable from the shipped binary. Fixed by wiring `include_next_action=True` into the real call site; new real-subprocess integration test (`test_prepare_real_cli_includes_next_action`) confirmed RED pre-fix, GREEN post-fix. The narrower "stop condition checks the wrong field" half of the claim (a dotted-path `ask_user_before_editing.required` mixed with a flat `result_incomplete` in the same `stop_if` list) was NOT resolved — left open below, lower confidence, needs the machine-protocol consumer contract clarified before deciding if it's actually wrong or an intentional mixed-lookup shape.
    - S6 — **CONFIRMED REAL, NOT YET FIXED (deliberately deferred).** Direct read confirmed both halves: `build_why_ranked_reasons` (`reranker.py:475`) only computes matched lexical terms + line span, never touches actual BM25/dense/RRF score contributions, so `--why-ranked`'s explanations are less than the feature name promises; `test_find_why_ranked.py:27` only asserts `isinstance(why_ranked, list)`, no content/quality check. This is a real SCOPE gap (the feature works, just narrower than advertised), not a broken contract like S1/S3 — fixing it correctly needs the per-component ranking scores threaded through to this function, which is real feature work, not a $0 same-turn fix. Scope as its own follow-up item when picked up.
    - S3's remaining `stop_if` field-shape question (above) — unresolved, low urgency, needs design clarification not a code fix.
  - **S4 is now merged (`4fc9b6b`); both remaining findings below were CONFIRMED REAL AND FIXED (`28584f4`) after S4 landed on `main`:**
    - ~~"refresh discards `last_prepare`, while resume still returns `resumed: true`"~~ — confirmed a DETERMINISTIC data-loss bug (not merely a race): `refresh_session` built its replacement payload from scratch every time, never carrying `last_prepare` forward. Fixed by carrying it forward from the existing payload when present.
    - ~~"unlocked prepare/refresh read-modify-write race"~~ — confirmed real: `session_prepare` locked on `root` itself (a key nothing else in the session subsystem used), while `refresh_session`'s payload write to the SAME file had NO lock at all. Fixed by moving both onto one shared `index_lock(_index_path(root))` acquisition (merged into refresh_session's existing index.json lock scope, since `index_lock` is not reentrant).
    - "'warm' prepare rescans instead of using the session map" — **CONFIRMED REAL, DELIBERATELY DEFERRED.** Direct read confirmed: `session_prepare` calls `build_prepare_snapshot(path=path, query=query)` with no way to pass the session's already-loaded `payload["repo_map"]`; `build_prepare_snapshot` itself has no cache-reuse parameter and always delegates to `_build_prepare_payload`, which per its own docstring does ONE full repo-map build from scratch. A "warm" session gets zero benefit — every `tg session prepare` call rescans as if cold. This is a genuine performance defect, not a correctness bug (results are still right, just wasteful) — properly fixing it means threading a cache-reuse path through the shared `_build_prepare_payload`/`build_prepare_snapshot` pipeline that `tg prepare` (the public CLI command) also depends on, which is real design work risking a regression in shared, heavily-used code if rushed. Scope as its own follow-up item (e.g. as part of P9's memory-resident daemon work, which already targets this exact class of warm-turn latency).
...
## Recent campaign notes (2026-08-22) - CEO-GATE COUNCIL

Full historical council record: [2026-08-22 CEO gate council](audits/2026-08-22-ceo-gate-council.md).
