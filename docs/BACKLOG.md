...
| **AGT-02** | PR #1136 OPEN (CI running): race fix (`611ca8d`) plus versioned `decision_generation`/`current_generation`/`decision_freshness` (current/historical/unknown) metadata on `last_prepare`, changed-content-retains-history, and legacy-payload-resolves-unknown behavior, all covered by new tests in `tests/unit/test_session_resume.py`. Flip to SHIPPED once merged. | [Task 02](plans/2026-09-07-agentic-quality-simplification.md#task-02) |
