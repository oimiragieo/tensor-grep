...
| F7 | Caller-graph parity for C/C++ | **Real and sized, smaller than before.** `_symbol_navigation_descriptor()`: 10 registered / 8 parser-backed. Java (PR #927, Task 10A), C# (Task 10B), and PHP (Task 10C) all shipped in-file parser-backed refs/callers; each one's cross-file caller confirmation still relies on the same text prefilter the other languages use, so a `resolution_gaps` entry still names that gap. `defs` exist for the remaining two (C/C++); agents key on the **blast floor**, which they lack. This is the honest content of the P3 "language coverage" item -- and note it *relocates* the gap into the tier competitors are measured on (Gortex's bespoke tier is ~30 with resolved edges). |
