...
| **AGT-07** | IN PROGRESS (`22cb25c`): `core/completeness.CompletenessEvidence` -- immutable, enum-typed (`ScanState`/`Cause`/`RetryKind`) record over the existing `unified_incomplete_envelope()` dict projection; `from_legacy_envelope()`/`to_legacy_dict()` round-trip byte-for-byte, proven against the frozen truth table (complete-empty-scan, display-cap-only vs file-scan-cap, deadline, unreadable-path, mixed-roots-partial, unknown-legacy-evidence) plus 2 mutation controls. Codex Luna audit round 1 caught a real bug (status=True + no cause collapsed to the fabricated string `"unknown"` instead of `None`) -- fixed, round 2 AUDIT_CLEAR. Neither producer (`main.py`, `mcp_server.py`) nor any consumer is migrated to the typed record yet -- that's the remaining, higher-risk half of Task 09. | [Task 09](plans/2026-09-07-agentic-quality-simplification.md#task-09) |
