...
| **P9** | IN PROGRESS: `docs/design/2026-09-07-cache-ownership.md` records the component-ownership map (repo_map/session/native-trigram/semantic-BM25 caches, keys, lifetimes, invalidation) and formalizes the explicit disposition for dormant `semantic_index.py` -- **RETAIN, library-only, not adopted** (zero production callers verified via `git grep`, reaffirming the 2026-08-01 dead-code campaign's prior retain decision), guarded by `tests/unit/test_semantic_index_dormant_disposition.py` so a new caller or a lost honesty docstring can't drift silently. Zero-redundant-warm-map-builds (the `session_prepare` -> `build_prepare_snapshot` -> full `build_repo_map` rescan-on-every-call gap) and generation-safe publication remain open -- both require refactoring shared `prepare_service.py`/`session_store.py` code, deferred to avoid rushing a regression. | [Task 06](plans/2026-09-07-agentic-quality-simplification.md#task-06) |
