...
- **[x] P3 — Standardize MCP Incompleteness Protocol Envelope**
  ...
  - **Receipt:** Merged PR #1135, squash SHA `5b8c85e`, CI was fully green pre-merge (40 pass, 0 fail, 0 pending). **Released as `v1.118.0`** — `Semantic Release` job succeeded, tag + GitHub release confirmed published 2026-09-06 08:40:30 UTC (verified: `git fetch --tags` shows `v1.118.0`, `gh api .../releases/latest` confirms it, not just inferred from CI green). **PyPI publish was still in progress as of this check** — `pypi.org/pypi/tensor-grep/json` showed `info.version: 1.117.0` (not yet `1.118.0`) since `publish-pypi` is sequenced after `publish-github-release-assets`, which was still running. Per this session's own release-mechanics discipline: version presence on PyPI (once it updates) is NOT sufficient either — verify all 4 expected artifact filenames (3 wheels + sdist) before calling this fully shipped. Root cause of an earlier scheduling delay (~50min gap between all CI prerequisites passing and the `Semantic Release` job actually starting): unclear, most likely GitHub-side runner-queue contention — no repo-level blocking cause was found on inspection. Worktree + branch cleaned up. **10 real defects found and fixed across 3 independent Codex Sol audit rounds** before merge, every one with a RED-test-first reproduction, not a self-report:
