...
**1 open PR draining: #570** index `.gitignore` non-git-dir no-op fix (#127, `add_ignore` trio in `index.rs`, Opus-gate SHIP, 5 Rust tests) -> **v1.70.1** (`fix` bump); CI matrix running, merge when green one-per-publish. **#569** de-flake already merged (`6eaf384`, `test:`, no release). Next after v1.70.1: docs PR (skills accuracy-refresh + this BACKLOG refresh). Before this wave: **#566** (`tg importers` outside-root honest signal -> v1.69.3, dogfood-verified live). Before it, the v1.68.1 CEO WSL-dogfood 3-PR drain (#562/#563/#564 -> v1.69.0/.1/.2) fully drained one-per-publish, zero broken releases; before it, campaign #142's 4-PR queue (#554-557) drained to v1.67.1/v1.68.0/v1.68.1/v1.68.2 — zero broken releases. Post-campaign artifacts also merged: **#559** backlog-reconcile + **#560** AGENTS.md ruff-scope docs-hardening (both docs-only, no release).
