...
- **[ ] AUDIT-FOLLOWUP — Unverified HIGH findings from 2026-09-06 Codex Sol session audit**
  ...
  - **On already-merged `main` code:**
    - S3 (`main.py`/`prepare_service.py`, PR #1132, merged `3e56c22`): "`next_action` is disabled on the public `tg prepare` path, and its stop condition checks the wrong field." Cited `prepare_service.py:256`, `prepare_service.py:460`, `main.py:6737`.
    - S6 (`reranker.py`, PR #1134, merged `d5c9354`): "explanations expose matching terms and line ranges, not actual ranking contributions; the test checks only list type." Cited `reranker.py:475`, `test_find_why_ranked.py:24`.
