...
| F7 | Caller-graph parity for C/C++ | **Real and sized, smaller than before.** `_symbol_navigation_descriptor()`: 10 registered / 9 parser-backed. Java (PR #927, Task 10A), C# (Task 10B), PHP (Task 10C), and C (Task 10D) all shipped in-file parser-backed refs/callers; each one's cross-file caller confirmation still relies on the same text prefilter the other languages use, so a `resolution_gaps` entry still names that gap. `defs` exist for the one remaining language (C++); agents key on the **blast floor**, which they lack there. This is the honest content of the P3 "language coverage" item -- and note it *relocates* the gap into the tier competitors are measured on (Gortex's bespoke tier is ~30 with resolved edges). |
