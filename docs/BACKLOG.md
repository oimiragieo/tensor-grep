...
- **[ ] P3 — Standardize MCP Incompleteness Protocol Envelope**
  ...
  - **Status:** PR #1135 open (`a7ad30e`..`4007527`). **9 real defects found and fixed across 3 independent Codex Sol audit rounds**, every one with a RED-test-first reproduction, not a self-report:
    ...
    - **Pattern worth naming explicitly:** each of the first 2 rounds looked complete until the next independent check found something real — see [[feedback-one-audit-round-is-not-enough-for-security-shaped-fixes-2026-09-06]]. This is now round 3 with the same shape. **Not unilaterally declaring merge-ready.** A 4th independent check (or an explicit operator risk-accept weighing depth-already-applied against diminishing returns) is the right next step — not another self-directed audit round from this same orchestrator. Verified so far: 17 envelope tests + 39 search tests + 61 contract-fix tests + 28 ratchet tests, all passing, each fix independently re-executed by the orchestrator (not trusted from a builder's report).
