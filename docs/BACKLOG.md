...
- **[ ] P3 — Standardize MCP Incompleteness Protocol Envelope**
  ...
  - **Status:** PR #1135 open (`a7ad30e`..`0e5e66b`), **STILL BLOCKED — two more real gaps found by a second-order (delta-verification) Codex Sol audit, after the first round of fixes.** History: round 1 found 45 raw `json.dumps` sites vs 35 injector-routed (fixed: 3 real bypasses — `tg_session_open`, `tg_session_list`, `tg_query` aggregate — now 38/42, independently spot-checked by Sol against 15 of the 42 remaining, all confirmed legitimate error-helper paths) and a `truncated`-signal blind spot in `unified_incomplete_envelope` (fixed `7cf88d4`). Round 2 (delta-verification) found the fix was still incomplete: `tg_query`'s `partial`/`omitted_roots` fields and nested `results_by_root[*].incomplete` children were also unchecked — fixed (`0e5e66b`), TDD-verified (13/13 envelope tests). **Remaining known-real, NOT yet fixed** (from the round-2 audit, not re-verified further this session — stopping the fix sweep here after 5 rounds of real, confirmed-then-fixed CRITICALs on this one PR):
    - ~~`tg_search(count_matches=true)` scan-cap path still reports complete when `max_repo_files` is hit~~ — **CONFIRMED REAL, FIXED (`15cb8b4`).** `count_payload` now stamps the same top-level `truncated` field the `is_empty` branch already did from the same `scan_limit_payload` value. TDD: 1 RED test (mocked 2-file walk, `max_repo_files=1`), confirmed `KeyError` pre-fix, green after; full search suite (39) + ratchet (28) still pass.
    - **STILL OPEN:** the envelope discards nested `scan_limit.truncation_cause`/`scan_limit.budget_remediable`, degrading an actionable cause into generic `"truncated"` and always reporting non-remediable (`mcp_server.py:3165-3177`). Not fixed this pass — needs `unified_incomplete_envelope` to also read `scan_limit.truncation_cause`/`.budget_remediable` when present, which is straightforward but wasn't re-verified against the current file state before this session closed.
    - HIGH, not CRITICAL: no ticket-identity/atomicity validation in the RELATED S1 edit-ticket service (see S1/S5 below) — noted here since the same audit surfaced it in the same pass.
