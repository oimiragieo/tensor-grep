...
| **AGT-08** | IN PROGRESS (`c366a34`): `core/retrieval_fusion.reciprocal_rank_fusion_explained()` delegates fused order to the existing `reciprocal_rank_fusion` (fuzz-proven byte-identical across 25 random ranking shapes, both combine modes -- explanation is a provable no-op on order) and returns a real per-leg-term `FusionExplanation` per chunk, including an explicit 0.0 floor for a leg that never ranked the chunk. Codex Luna audit round 1 caught two real defects (duplicate-chunk-id-in-one-leg divergence from the plain function's accumulation; silent duplicate-`leg_names` overwrite), both fixed (exact per-occurrence accumulation, `leg_names` uniqueness validated), round 2 AUDIT_CLEAR. Wiring this into `tg search`'s CLI/MCP output and the production BM25+dense retrieval call sites remains open. | [Task 11](plans/2026-09-07-agentic-quality-simplification.md#task-11) |
