...
| **AGT-03** | IN PROGRESS (`7ae7063`): `tg prepare --include-next-action` no longer hardcodes `uv run pytest -q` for every repository; `on_success.argv` is now selected only from an exact-match allowlist of the capsule's own detected `validation_commands`, falling back to `on_success.unavailable` (never a shlex.split of unreviewed shell text) when nothing matches. Typed `ValidationAdvice` record, `prepare_protocol.py` module, centralized `stop_if` (non-current-decision), and versioned canonical session serialization remain open. | [Task 03](plans/2026-09-07-agentic-quality-simplification.md#task-03) |
