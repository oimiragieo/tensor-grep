...
> Last refreshed 2026-07-12 (steward tick; v1.66.1 live, v1.67.0 building — evidence-signing + release-blocker hotfix).
...
- **Live PyPI v1.66.1**. **v1.67.0 building** (main `35868fa`; content = **#553** Ed25519 evidence-signing — `tg evidence emit --sign/--verify/keygen/pubkey`, adds `cryptography>=48.0.1` — plus **#558** release-blocker hotfix: ruff-formatted the cluster-124 doc's embedded Python code fence + regenerated `uv.lock`, because #553 had merged with 2 RED release-gating checks (static-analysis + dep-audit). Release run queued behind GitHub-Actions runner backlog. Root-cause + traps banked: memory `tensor-grep-whole-repo-ruff-format-gap-and-git-show-smudge-2026-07-12`.
- **4 open PRs, draining one-per-publish** (WIP=4, under the 5-PR cap): **#554** MCP `tg_session_open` `max_repo_files` 512→2000 (#98, **gate-cleared** benign default bump) — drains FIRST at PyPI=1.67.0 (poller armed). **#555** daemon Tier-2 serving orient/agent capsules (#108, ~20x latency: orient 14.6s→0.95s / agent 21.5s→1.5s) — Opus RE-gate running (a prior gate caught + the builder fixed a cold-path literal-seed-rescue recall regression). **#556** apply_policy canonicalize-exec-parent (#126, closes 8.3/junction/`\\?\` fail-open edges) — mandatory Opus gate running. **#557** `--count-matches` refuses cleanly when rg unresolvable instead of silently printing a wrong line-granular count (#121) — Opus gate running. NOTE: the 3 build PRs currently fail static-analysis ONLY on the *inherited* cluster-124.md (fixed by #558 on main → a rebase clears them).
...
**4 open PRs — WIP=4** (drain one-per-publish; details in CURRENT STATE): **#554** mcp 512→2000 (gate-cleared, drains first at PyPI=1.67.0, poller armed) · **#555** daemon Tier-2 capsules (Opus re-gate running) · **#556** apply_policy canonicalize (Opus gate → **FIX-FIRST**: gate caught a 4th UNC-admin-share bypass, builder applying the UNC-denial fix) · **#557** count-matches degrade (Opus gate running). Drain tracked as task #148.

## SHIPPED — live on PyPI up to **v1.66.1**; v1.67.0 building
