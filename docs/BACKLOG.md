...
- **[ ] P3 — Standardize MCP Incompleteness Protocol Envelope**
  ...
  - **Status:** PR #1135 open (`a7ad30e`..`0b949f0`). **All findings from both Codex Sol audit rounds are now fixed and TDD-verified** (round 1: 3 injector-routing bypasses + `truncated`-signal blind spot; round 2: `partial`/nested-aggregation gap, `tg_search` count-mode scan-cap gap, `scan_limit.truncation_cause`/`.budget_remediable` precision gap — 7 real defects total across the campaign, every one with a RED-test-first reproduction). **Not yet independently re-verified by a third audit pass** — per this session's own banked lesson ([[feedback-one-audit-round-is-not-enough-for-security-shaped-fixes-2026-09-06]]), two rounds were each individually insufficient on this exact PR, so treat "all known findings fixed" as necessary but not sufficient for merge confidence. Recommend: merge once CI is green AND either (a) a third Sol pass comes back clean, or (b) the operator accepts the risk given the depth already applied (15 envelope tests + 39 search tests + 28 ratchet tests, all passing, each fix independently reproduced RED-then-GREEN by the orchestrator, not just self-reported by a builder).
