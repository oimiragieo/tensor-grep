...
- **[x] S1/S5 — Fail-Closed Edit Tickets & Verify-Edit Service (`EditReadyTicketV1`)**
  ...
  - **CRITICAL follow-up (2026-09-06, confirmed real by independent Codex Sol audit + direct code read):** `verify_edit_ticket` only checked that the caller-supplied `modified_files` list was a subset of `allowed_files` — it never recomputed `pre_edit_fingerprints`/`working_tree_fingerprint` against actual tree state, so it could not catch an undeclared out-of-scope modification or a hallucinated (never-applied) declared edit. The service was confirmed unwired (`grep -rln verify_edit_ticket src/` = only the file itself) so no live blast radius, but the core contract was broken on `main`. Fixed `86caffc`: `build_edit_ready_ticket` now fingerprints the whole tree (not just `allowed_files`); `verify_edit_ticket` recomputes and diffs. TDD: 2 new RED tests confirmed both gaps pre-fix, 6/6 green after.
...
- **[ ] P3 — Standardize MCP Incompleteness Protocol Envelope**
  ...
  - **Status:** PR #1135 open (`a7ad30e`..`7cf88d4`), **BLOCKED — do not merge — the acceptance criterion is not yet true.** An independent Codex Sol audit (`docs/audits/codex-specs/2026-09-06-session-fanout-audit.md`) found the "all 58 MCP tool endpoints" claim was false: a direct count showed 45 `json.dumps` return sites in `mcp_server.py` vs only 35 routed through the `_inject_mcp_contract_fields` injector (confirmed real by direct read: `tg_session_open`/`tg_session_list` build their success response with a bare `json.dumps`, never touching the injector). A second CRITICAL, also confirmed real: `unified_incomplete_envelope()` keyed only on `result_incomplete`, so a response truncated via the separate `truncated`/`scan_capped` signal (e.g. `tg_search`'s no-match envelope, `mcp_server.py:3649`) could report `incomplete.status=False` while genuinely incomplete — fixed (`7cf88d4`), TDD-verified (9/9 envelope tests). A third CRITICAL was already fixed pre-audit (the `tg_mcp_capabilities` bare-call regression, `5e67ade`). **Remaining work dispatched** to a Sonnet-pinned builder agent: audit all 45 sites, fix genuine success-path bypasses (session tools confirmed, others TBD), leave legitimate error-payload helpers alone, full TDD verification — in flight, no result yet. Do not merge until that lands and its own claims are re-verified the same way (this item has now been wrong about its own completeness twice).
