...
agent — "the product wedge is **not** 'faster grep'" (`AGENTS.md:159`) — so a result that *looks* right but
...
- **The negative controls.** A no-match is a real outcome your mechanism must survive, not an error to hide.
  ...
  `tg` also returns zero matches (`AGENTS.md:155`) — a mechanism that only "works" on matching inputs and
...
- **The edge/adversarial inputs.** CRLF, invalid UTF-8, BOM, binary/NUL-byte files, multiline. `rg`'s
  ...
  that swaps engines changes *results*, not just speed (`AGENTS.md` Backend Fail-Closed Contract, `:214-224`).
...
test**, because relaxing masks the real degradation (`AGENTS.md:166`).
...
> **A finding or claim with no `file:line` citation is DISCARDED** (`AGENTS.md:212`).
...
1. **Pre-build planning review / council** — before you implement, an independent review cites `file:line`
   for every seam claim in the plan; uncited claims are hypotheses, not facts (`AGENTS.md:208,:266`). A
...
2. **Post-build adversarial audit** — a mandatory, *separately named* stage that adversarially reviews the
   ...
   zero-finding state is the convergence gate before a draft PR (`AGENTS.md:212,:271`). This stage once
...
~2.3x slower (`tensor-grep-benchmark-and-proof-toolkit` worked example; `AGENTS.md:156` names the fair
...
hypothesis until it cites a `file:line` that actually resolves** (`AGENTS.md:204-212`). AI plans have a
...
- **Never trust a self-report.** A subagent's "tests pass" / "N green" is a hypothesis until *external state*
  ...
  literally un-runnable in their own tree (`AGENTS.md:210`; `tensor-grep-change-control` Part 1).
- **Green ≠ working when the test never touches the real boundary.** Mock-based FFI tests were green while
  ...
  `os.replace`), not by reading substrings (`AGENTS.md:210`).
...
currently *slower* than CPU and its P1 CUDA kernel is **paused**, `AGENTS.md:226-234`). "Experimental" is a
...
- **Weight ideas by the strategy, not by novelty.** Raw search speed is the **parity tier**; the moat is the
  **agent-native context layer** (`orient` / `callers` / blast-radius / the token-efficient capsule)
  (`AGENTS.md:159,:226-234`). A clever idea that only makes cold grep marginally faster is off-strategy even
  if it works.
- **A validated user ask outranks a speculative one.** "Local hybrid semantic search" is the top roadmap
  item because it is the **#1 validated user ask and the biggest competitive gap** (`AGENTS.md:230`), not
...
Volatile facts are dated **2026-07-02, release `v1.17.25`**. Re-verify before relying on any of them — a
wrong methodology runbook lets a bad result through, which is worse than none.

| Claim | Re-verify command |
|---|---|
| Current release tag | `grep -n release_docs_current_tag AGENTS.md` (currently `v1.17.25`) |
| "Benchmark-governed, do not optimize by guesswork" | `grep -n "benchmark-governed" AGENTS.md` (`:15`) |
| Product wedge is not "faster grep" | `grep -n "not \"faster grep\"\|agentic code-intelligence" AGENTS.md` (`:159`) |
| Verify-plan + adversarial-audit + "no citation is DISCARDED" | `grep -n "DISCARDED\|ADVERSARIAL AUDIT\|caught 5 blockers\|CUDA-fork hazard" AGENTS.md` (`:208,:210,:212,:266,:271`) |
...
| No-match is a valid comparator outcome; fair many-fixed-strings baseline | `grep -n "no-match as a real comparator\|many fixed strings" AGENTS.md` (`:155,:156`) |
| Ranking flip: harden, don't relax the test | `grep -n "IDF blast-radius\|robust to IDF shifts" AGENTS.md` (`:166`) |
| Roadmap sequencing (3 CPU wins; GPU paused; Semble; #1 user ask; registration-check) | `grep -n "Roadmap Sequencing" -A 12 AGENTS.md` (`:226-234`) |
| Security round-3 as an audit lens | `grep -n "Security Hardening Patterns" -A 8 AGENTS.md` (`:236-243`) |
