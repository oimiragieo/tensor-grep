...
**Why current SOTA fails.** `tg`'s ranking-dependent surfaces — the agent capsule, `tg search --rank`, local semantic search — score with `_score_text_terms` (`src/tensor_grep/cli/repo_map.py`), a **flat presence count with NO IDF** (no term-rarity weighting). Combined with a hard top-N candidate cap and a final tie-break on the alphabetical **file-path string** (`_symbol_rank_key`, same file), any corpus change can reweight a query, **flip** the primary target, and silently **degrade a safety behavior** — with **zero edge in the call graph**, so neither `tg callers` nor blast-radius can see it (AGENTS.md, the "BM25/IDF-ranked surfaces are sensitive to corpus changes" bullet). This corpus-relative fragility is the "blast-radius beyond the call graph".

**Do not re-fight the settled part.** A thinktank council **disproved** the naive "just add IDF" guess as *the* fix — the real chain is flat-scorer + top-N cap + alphabetical tie-break, so a minimal "widen the pool" patch was *proven insufficient* (see `tensor-grep-failure-archaeology`). What already shipped in **v1.17.13 (#302)**: a **degrade-to-ask safety floor** (`_primary_target_is_unrequested_marker_helper` forces `ask_user=True` when the post-swap primary is still an unrequested marker), `_prefer_implementation_over_marker_helper`, and migration of the exact-identity contract to a deterministic controlled-corpus fixture. **The flat scorer itself remains** — that is the open frontier.
