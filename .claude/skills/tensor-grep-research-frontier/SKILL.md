...
Date-stamped **2026-07-08, v1.49.3** (originally authored 2026-07-02 at v1.17.25). Re-verify volatile
facts with the commands in **Provenance and maintenance** before you cite them.
...
**Falsifiable sub-item — `#74`, the scoped file-dependency primitive (benchmark-validated demand, 2026-07-08).** The first real token-economy proof-point (Sverklo `bench:primitives`, oracle-validated tokens-per-correct-answer on `express@4.21.1`) found `tg` **7.5x better than grep on definition-lookup** but roughly an **order of magnitude worse on file-deps** ("what does file X import" / "who imports file X"), because `tg` has no scoped file-dependency primitive — an agent pays a whole-repo `tg map` to answer a single-file question (memory `tensor-grep-benchmark-proofpoint-2026-07-08`; re-run the benchmark before citing an exact multiplier, the committed-artifact number and the memory number have not yet been reconciled as of this writing — `docs/benchmarks.md`). A code-grounded Fable design for the fix exists (`tg imports <file>` / `tg importers <file>`, two commands mirroring the `callers` shape: forward resolution reuses `_imports_and_symbols_for_path`; reverse reuses the existing but over-counting `_reverse_importers` alias path strictly as a *prefilter*, confirmed per-edge against the real module-matchers to avoid inheriting its false-edge bug) but **no code has been written** — this is 100% open/candidate, not started. **Result when:** `tg imports`/`tg importers` ships behind the normal 4-site registration + MCP tools, and a re-run of the Sverklo file-deps task slice shows tokens-per-correct-answer within roughly the same order of magnitude as the definition-lookup win, not 10x worse.
...
### 4d. Local agent context-sharing / shared code-intelligence plane (A2A landscape, added 2026-07-08)
**Why current SOTA is crowded, not empty.** A 2026-07-08 landscape review (thinktank-reviewed, memory
`tensor-grep-a2a-ledger-audit-2026-07-08`) found MCP (agent-tool) and A2A (cross-org agent-agent,
HTTP/OAuth) already settled at the protocol layer, and the *local* agent-coordination layer exploded
in Q1 2026 and is now harness-owned (Claude Code Agent Teams, Beads) — all of it trading in **prose**
task state. The one sliver still off everyone else's moat is **code-aware** coordination: repo maps
and blast radii that only a tool like `tg` actually computes. `tg` is already ~80% of the way to a
local shared-context plane via its existing cross-process session store (`<repo>/.tensor-grep/sessions/`)
and the token-authed loopback daemon (`session_daemon.py`) that lets a second agent reuse a first
agent's warm repo-map/context-render/blast-radius work. **tg asset:** `session_store.py`,
`session_daemon.py`, the existing `tg session daemon start/status/stop` surface — no new protocol.
**Status — CEO-gated, DOCUMENT-NOW-BUILD-LATER-ON-GATE (do not build a ledger/claims/messages layer
without a demand receipt):** the approved step-0 is docs-only (a new `docs/multi_agent_context_plane.md`
positioning page) plus a small opt-out, fail-open, PII-free demand-instrumentation patch on the
session daemon (concurrent-distinct-client counter + repeat-expensive-artifact counter, hashed
targets only, read back via `tg session daemon status --json` / `tg doctor --json`) that earns (or
fails) a 2-week gate before any real build is authorized. **This step-0 work is merge-gated, not
shipped** — it landed as `#456`, which was **open, not on `main`**, at the time this section was
written; re-verify with `git log --oneline origin/main | head` (look for a `chore(release)` commit
that supersedes it) before treating the docs page or the instrumentation as live. **Result when
(falsifiable):** the 2-week window ends with either (a) the pre-stated gate MET — e.g. >=3 days with
>=2 concurrent distinct clients AND >=5 duplicate-artifact computations — which authorizes designing
the next layer, or (b) NOT-MET, which is itself a valid, recorded result (document-only was correct,
learned for near-$0). Do not build the ledger/claims layer on a hunch either way.
...
- **Version / date stamp** (`v1.49.3`, 2026-07-08): `grep -n '^version = ' pyproject.toml` and `grep -n 'release_docs_current_tag' AGENTS.md`.
...
- **Problem 4d (A2A/local-shared-context) + Problem 4b sub-item (`#74` scoped file-deps):** design docs `tensor-grep-a2a-ledger-audit-2026-07-08` and `tensor-grep-benchmark-proofpoint-2026-07-08` memories; the in-repo docs page (`docs/multi_agent_context_plane.md`) and the demand-instrumentation seam are on the **open, unmerged** `#456` PR at time of writing — `git log --oneline origin/main | head` to confirm merge/release state before citing either as shipped.
