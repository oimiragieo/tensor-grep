...
without a demand receipt):** the approved step-0 is docs-only (`docs/multi_agent_context_plane.md`
...
fails) a 2-week gate before any real build is authorized. **This step-0 work is MERGED (`#456`,
commit `fca77a4`, 2026-07-08)** -- the docs page and the instrumentation are live on `main`; the
2-week demand-gate window now runs from that merge date. Re-verify current elapsed time and counter
values with `tg session daemon status --json` / `tg doctor --json` before citing the gate as met or
unmet. **Result when
