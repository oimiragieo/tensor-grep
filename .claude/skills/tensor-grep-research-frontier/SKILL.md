...
5. **Never trust a self-report.** A subagent's "it works / tests pass" is a hypothesis until an exit code, a real-binary dogfood, or a `file:line` that resolves confirms it. This cuts both ways: the plan going IN needs the same skepticism as the report coming out. Receipt (2026-07 language-expansion campaign): an orchestration brief that said "mirror inline `_rust_*`" was itself stale — the repo had already grown the `lang_registry` module pattern — and it only survived because every dispatched build agent re-verified the brief against current code before touching a file, not because the brief was trustworthy on its face. See AGENTS.md "Verify AI-Drafted Plans Against the Real Code Before Building."
...
**Reconciled (2026-07-24) — do not read the funding note above as "still hypothetical."** The CPU
semantic direction it references has, in the time since, itself shipped: `tg search --semantic` (BM25 +
local CPU dense embeddings, RRF-fused; `src/tensor_grep/core/retrieval_dense.py` +
`retrieval_fusion.py`) is live on `main`, not merely funded — `tensor-grep-semantic-search-campaign`'s
own status line says it plainly ("SHIPPED as `tg search --semantic`"). Read "funded engineering
priority" above as resourcing history, not a claim the capability is still pending. Separately, the GPU
surface now has its own day-to-day operator skill, `tensor-grep-gpu` (device/`doctor` probes,
`--gpu-device-ids` search, `calibrate`) — go there for the operational commands; this Problem stays the
open crossover *research* question, the same split Problem 4d uses to hand its operator surface to
`tensor-grep-ledger`.
