...
Verified against **tg 1.95.0** (last full WSL workspace+GPU dogfood 2026-07-21 at v1.91.0; individual
gaps below re-verified by source inspection against the shipped v1.91.1-v1.95.0 line, not a re-run
whole-workspace sweep — see the native-scale dogfood bullet below for a fresh large-repo data point).
...
| Symbol-graph language coverage | Partial (8/10 top-10 languages register via `lang_registry` — Python/JS/TS/Java/C#/Go/Rust/PHP, each correctly reporting `primary_target_language` for the capsule confidence gate; C/C++ deferred — text search and `ast-info`/LSP still cover them, just not the deep symbol/caller graph) |
