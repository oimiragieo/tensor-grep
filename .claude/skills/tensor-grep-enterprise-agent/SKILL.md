...
| Symbol-graph language coverage | **REGISTRY 10/10; CALLER-GRAPH 4/10 -- quote the split, never the bare 10/10.** All ten register (`c, cpp, csharp, go, java, javascript, php, python, rust, typescript`; C/C++ via `lang_c.py`/`lang_cpp.py`). But this skill's own hard-stops key on `blast_radius_floor`/`callers_count`, and those need the AST caller path: FULL for python/javascript/typescript/rust, PARTIAL for go (`references_and_calls` only), FOUNDATIONAL-TIER for java/php/csharp/c/cpp -- defs+imports are real, while `tg refs`/`callers`/`blast-radius` fall through to the regex heuristic. Verify before quoting a number: count the `references_and_calls=None` specs in `repo_map.py` (5 of 8 registry specs today). Accepted ceiling: `class MACRO Name` in C++ misparses -- do not re-chase |
