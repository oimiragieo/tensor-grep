...
- **Ruled out:** the native Rust `tg` binary (`resolve_native_tg_binary`) is never reachable for `--stats` on either platform — `_can_delegate_to_native_tg_search`'s `unsupported_flags` explicitly lists `--stats` (`bootstrap.py:529`, unchanged this pass), and the `TG_RUST_FIRST_SEARCH` OR-branch (`grep -n "_prefer_rust_first_search() and not _requires_full_cli" src/tensor_grep/cli/bootstrap.py` — was `:1525`, now `:1547`) requires `not _requires_full_cli(...)`, which is always `False` for `--stats` (`_TG_ONLY_SEARCH_FLAGS`, `bootstrap.py:50`, unchanged this pass). `cli/mcp_server.py` has no `can_passthrough_rg`/`search_passthrough` shortcut at all — this divergence is CLI-only.
