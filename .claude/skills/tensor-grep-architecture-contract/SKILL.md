...
`tg` is not "a Typer app." The published entry point is `bootstrap.main_entry` (`src/tensor_grep/cli/bootstrap.py:1398` -- re-grep `^def main_entry`, this has drifted twice). It parses `argv` itself and, for a **plain text search**, forwards to the native `tg` binary or to ripgrep *before Typer ever runs* (`bootstrap.py:1187-1260` — from the `_normalize_search_invocation` call through the final `_run_rg_passthrough` dispatch, traced and confirmed this pass). The Typer app is only reached for TG-only flags, help, or commands that require full CLI (`_requires_full_cli`, `bootstrap.py:396`).
