...
`tg` is not "a Typer app." The published entry point is `bootstrap.main_entry` (`src/tensor_grep/cli/bootstrap.py:1444` -- re-grep `^def main_entry`, this has drifted twice). It parses `argv` itself and, for a **plain text search**, forwards to the native `tg` binary or to ripgrep *before Typer ever runs* (re-grep `_run_rg_passthrough` — the dispatch runs from the `_normalize_search_invocation` call through the final `raise SystemExit(_run_rg_passthrough(...))`; traced live 2026-07-29 and confirmed a bare `tg search PAT` exits THERE, never reaching Typer). The Typer app is only reached for TG-only flags, help, or commands that require full CLI (`_requires_full_cli`, `bootstrap.py:396`).
...
- **Two mutual-delegation fork-bomb hazards are guarded, not theoretical.** `TG_REEXEC_GUARD` (checked at `bootstrap.py:1497`) stops native→python→native search loops; `_json_aggregate_blocks_passthrough` (`bootstrap.py:488`) stops `--json` + a render-only flag (e.g. `-b`) from deadlocking the native front door; `_run_requires_ast_workflow` (`bootstrap.py:1385`) keeps `tg run --selector/--strictness/--stdin/--globs` in Python so it does not ping-pong. If you touch delegation, you can re-arm a fork bomb — see `tensor-grep-failure-archaeology`.
...
- **Parse-first-then-branch** — `backends/ripgrep_backend.py` (`search`, `_search_files_with_matches`, `_search_counts`): exit 2 with a non-empty parse *keeps* the results, sets `result_incomplete=True` + a stderr-derived `incomplete_reason` (`ripgrep_backend.py:145-150,326,445`); exit >2, or exit 2 with nothing parsed, raises `BackendExecutionError` (**RESOLVED #79/#10/#14, commit `a7c9431`**: every `RipgrepBackend` fatal path, including the rg-missing guard at `:505`, now raises `BackendExecutionError` instead of a bare `RuntimeError`, so `cli/main.py`'s per-file `except BackendExecutionError` CPU-fallback retry (`:8005`) catches it instead of falling into the broad `except Exception` and crashing the whole search — see `code-search-and-retrieval-reference` §1 for the exit-code table).
- **Monotonic merge** — `merge_runtime_routing` (`core/result.py:135+`) OR-merges `result_incomplete` across sub-results (`aggregate.result_incomplete or result.result_incomplete`), so the CLI/MCP/sidecar aggregate inherits uniformly — any incomplete sub-result taints the whole.
...
- `can_passthrough_rg` (`cli/main.py:7920-7936`, built by `_can_passthrough_rg`, `cli/main.py:5342-5379`) categorically excludes `--ast`/`--rank`/`--semantic` (`not config.ast` / `not config.rank_bm25` / `not config.semantic_rank`, `cli/main.py:5359-5363`) but has **no equivalent categorical exclusion for `--stats`** — its only stats-specific veto, `not (rg_json_passthrough and stats_mode)` (`cli/main.py:5373`), fires solely for the `--format rg --json` combo. A plain-text `tg search PAT --stats` sails through.
- When `stats=True`, `search_command` has its own branch for it (`cli/main.py:8004-8017`):
  ```python
  if can_passthrough_rg and stats and _selected_route_supports_rg_passthrough(...):
      ...
      sys.exit(exit_code)
  ```
  This hands the **entire** search to a live `rg` subprocess — `RipgrepBackend._build_cmd` appends rg's own `--stats` flag when `config.stats` is set (`backends/ripgrep_backend.py:805-806`) — and exits on rg's own exit code, entirely bypassing everything `search_command` would otherwise do downstream: its own `_emit_stats()` `[stats] backend=... reason=...` line (`cli/main.py:8371-8407`), the `--debug` routing echo (`cli/main.py:8040-8043`), and — the reported symptom — the `is_empty` branch's defaulted-scope note (`cli/main.py:8437-8476`, `_write_defaulted_scope_note`).
- `_selected_route_supports_rg_passthrough` (`cli/main.py:5451-5462`) requires `Pipeline.selected_backend_name == "RipgrepBackend"`, which `core/pipeline.py`'s backend-selection `__init__` picks via `elif rg_available: self.backend = rg_backend; selected_backend_reason = "rg_default_fast_path"` (`core/pipeline.py:356-359`) whenever `rg_backend.is_available()` is `True`. That in turn is `RipgrepBackend.is_available()` → `resolve_ripgrep_binary()` (`backends/ripgrep_backend.py:45-52`, `cli/runtime_paths.py:561-591`) — a pure **environment** probe: `TG_RG_PATH` env var, then `shutil.which("rg"/"rg.exe")` on `PATH`, then an in-tree fallback directory that is **gitignored** (`ripgrep-*/` in `.gitignore`) and absent from a fresh checkout on any platform (only the zip `benchmarks/rg.zip` is committed, and only a separate *test* helper — `tests/helpers/rg_parity.py::resolve_pinned_rg_binary` — knows how to unpack it; production code never touches the zip).
- **There is no `sys.platform`/`os.name` conditional anywhere in this chain.** The Windows-vs-Linux CI split is caused entirely by whether a real `rg`/`rg.exe` happens to be resolvable on `PATH` inside each OS leg of the `test-python` CI job (`.github/workflows/ci.yml:361-430`), which installs **no** ripgrep package on any OS — contrast the `native-build-smoke`/`smoke`/`benchmark` jobs, which explicitly `apt-get install ripgrep` / `brew install ripgrep` (`.github/workflows/ci.yml:673-685,780-783`).
...
- **Ruled out:** the native Rust `tg` binary (`resolve_native_tg_binary`) is never reachable for `--stats` on either platform — `_can_delegate_to_native_tg_search`'s `unsupported_flags` explicitly lists `--stats` (`bootstrap.py:529`), and the `TG_RUST_FIRST_SEARCH` OR-branch (`bootstrap.py:1525`) requires `not _requires_full_cli(...)`, which is always `False` for `--stats` (`_TG_ONLY_SEARCH_FLAGS`, `bootstrap.py:72`). `cli/mcp_server.py` has no `can_passthrough_rg`/`search_passthrough` shortcut at all — this divergence is CLI-only.
- **Downstream blast radius found while sweeping (not fixed here — out of scope for the platform-divergence diagnosis):** the same `can_passthrough_rg` fork also gates the plain, non-stats internal passthrough (`cli/main.py:7937-7943`), and `RipgrepBackend._build_cmd` never translates `config.quiet` into rg's own `-q`/`--quiet` anywhere (zero mentions of "quiet" in `backends/ripgrep_backend.py`). A `tg search PAT --stats --quiet` whose `Pipeline` selects `RipgrepBackend` therefore still prints rg's live stats block (and any matches) to stdout, silently breaking `--quiet`'s "no incidental output" promise that the slow path enforces explicitly (`and not quiet` at the `is_empty` note, `cli/main.py:8468`; the `if quiet: sys.exit(...)` gate, `cli/main.py:8478-8480`). Untested (`--stats --quiet` appears nowhere under `tests/`) and **not platform-specific** — it reproduces wherever `rg` is installed, including ordinary user machines.
...
(`_select_ast_backend_for_pattern`, `cli/main.py:6737` -- cite by symbol; the old `:6690-6707` range now holds unrelated #299 code), not a hardware gate. The cpu_backend.py ReDoS-gate cites (§7 below) and the
