...
`tg` is not "a Typer app." The published entry point is `bootstrap.main_entry` — `grep -n "^def main_entry" src/tensor_grep/cli/bootstrap.py` (was `:1444`, now `:1466`; this anchor has drifted three times, never cite the number as fact). It parses `argv` itself and, for a **plain text search**, forwards to the native `tg` binary or to ripgrep *before Typer ever runs* (re-grep `_run_rg_passthrough` — the dispatch runs from the `_normalize_search_invocation` call through the final `raise SystemExit(_run_rg_passthrough(...))`; traced live 2026-07-29 and confirmed a bare `tg search PAT` exits THERE, never reaching Typer). The Typer app is only reached for TG-only flags, help, or commands that require full CLI (`grep -n "^def _requires_full_cli" src/tensor_grep/cli/bootstrap.py` — `:396`, unchanged this pass).

Why this matters, concretely:

- **CliRunner cannot see routing bugs.** It invokes the Typer app directly, so any bug in `bootstrap` routing (a flag that leaks to `rg`, a fork-bomb delegation loop, a wrong native/Python choice) is **invisible** to CliRunner tests and green in CI while broken for real users. This is exactly how the `--rank` plain-text crash shipped (`AGENTS.md` §"Dogfood the Real Binary, Not CliRunner" — `grep -n "^## Dogfood the Real Binary"` — was `:422`, now `:578`). **Rule: verify front-door behavior against the REAL published binary** via `scripts/dogfood/` (Dockerfile + `dogfood_features.py`), never CliRunner alone.
- **Two mutual-delegation fork-bomb hazards are guarded, not theoretical.** `TG_REEXEC_GUARD` (`grep -n 'os.environ.get("TG_REEXEC_GUARD")' src/tensor_grep/cli/bootstrap.py` — was `:1497`, now `:1519`) stops native→python→native search loops; `_json_aggregate_blocks_passthrough` (`grep -n "^def _json_aggregate_blocks_passthrough" src/tensor_grep/cli/bootstrap.py` — `:488`, unchanged this pass) stops `--json` + a render-only flag (e.g. `-b`) from deadlocking the native front door; `_run_requires_ast_workflow` (`grep -n "^def _run_requires_ast_workflow" src/tensor_grep/cli/bootstrap.py` — was `:1385`, now `:1407`) keeps `tg run --selector/--strictness/--stdin/--globs` in Python so it does not ping-pong. If you touch delegation, you can re-arm a fork bomb — see `tensor-grep-failure-archaeology`.
...
This is a **universal bug class**: "register in N places, miss one, fail *quietly*." The CI registration-completeness gate has been **BLOCKING since v1.17.1 / #282** (`AGENTS.md`, `grep -n "BLOCKING since v1.17.1"` — was `:414`, now `:516`), but you still author all sites by hand.
...
- **Parse-first-then-branch** — `backends/ripgrep_backend.py` (`search`, `_search_files_with_matches`, `_search_counts`): exit 2 with a non-empty parse *keeps* the results, sets `result_incomplete=True` + a stderr-derived `incomplete_reason` (`grep -n "result_incomplete = True" src/tensor_grep/backends/ripgrep_backend.py` — `:144,325,444`, essentially unchanged this pass); exit >2, or exit 2 with nothing parsed, raises `BackendExecutionError` (**RESOLVED #79/#10/#14, commit `a7c9431`**: every `RipgrepBackend` fatal path, including the rg-missing guard — `grep -n "requires the 'rg' binary" src/tensor_grep/backends/ripgrep_backend.py` (was `:505`, now `:541`) — now raises `BackendExecutionError` instead of a bare `RuntimeError`, so `cli/main.py`'s per-file `except BackendExecutionError` CPU-fallback retry — `grep -n "except BackendExecutionError" src/tensor_grep/cli/main.py` (was `:8005`, several call sites exist today, e.g. `:4796`/`:8280`/`:8396`; re-grep and confirm which one is the per-file retry before citing a single line) — catches it instead of falling into the broad `except Exception` and crashing the whole search — see `code-search-and-retrieval-reference` §1 for the exit-code table).
- **Monotonic merge** — `merge_runtime_routing` (`grep -n "^def merge_runtime_routing" src/tensor_grep/core/result.py` — was `:135`, now `:142`) OR-merges `result_incomplete` across sub-results (`aggregate.result_incomplete or result.result_incomplete`), so the CLI/MCP/sidecar aggregate inherits uniformly — any incomplete sub-result taints the whole.
...
- **JSON/NDJSON envelope** — `cli/formatters/json_fmt.py` (`grep -n "result_incomplete" src/tensor_grep/cli/formatters/json_fmt.py`; the `:126-127` emission pair is unchanged this pass, the second `:189-190` cite has drifted — re-grep rather than trust either): `result_incomplete`/`incomplete_reason` are emitted **only when incomplete**, so a complete result's JSON shape stays byte-identical to before #341.
...
`_can_delegate_to_native_tg_search` — `grep -n "^def _can_delegate_to_native_tg_search" src/tensor_grep/cli/main.py` (was `:3709`, now `:3794`) — gates whether a Python-side `tg search` hands the **entire** search to the native `tg` subprocess (`_build_native_tg_search_command` — `grep -n "^def _build_native_tg_search_command" src/tensor_grep/cli/main.py`, was `:3731`, now `:3816`) and then `sys.exit()`s on its result — a delegation that runs *before* the Python-side BM25 rerank (`--rank`) and the in-backend sort (`--sort-files`) ever execute.
...
- When `stats=True`, `search_command` has its own branch for it (`grep -n "_selected_route_supports_rg_passthrough(" src/tensor_grep/cli/main.py` — the call site was `:8004-8017`, now `:8102-8118`):
  ```python
...
- **Ruled out:** the native Rust `tg` binary (`resolve_native_tg_binary`) is never reachable for `--stats` on either platform — `_can_delegate_to_native_tg_search`'s `unsupported_flags` explicitly lists `--stats` (`bootstrap.py:529`, unchanged this pass), and the `TG_RUST_FIRST_SEARCH` OR-branch (`grep -n "_prefer_rust_first_search() and not _requires_full_cli" src/tensor_grep/cli/bootstrap.py` — was `:1525`, now `:1547`) requires `not _requires_full_cli(...)`, which is always `False` for `--stats` (`_TG_ONLY_SEARCH_FLAGS`, `bootstrap.py:72`, unchanged this pass). `cli/mcp_server.py` has no `can_passthrough_rg`/`search_passthrough` shortcut at all — this divergence is CLI-only.
- **Downstream blast radius found while sweeping (not fixed here — out of scope for the platform-divergence diagnosis):** the same `can_passthrough_rg` fork also gates the plain, non-stats internal passthrough (`grep -n "if can_passthrough_rg:" src/tensor_grep/cli/main.py` — was `:7937-7943`, now `:8037-8042`), and `RipgrepBackend._build_cmd` never translates `config.quiet` into rg's own `-q`/`--quiet` anywhere (zero mentions of "quiet" in `backends/ripgrep_backend.py`). A `tg search PAT --stats --quiet` whose `Pipeline` selects `RipgrepBackend` therefore still prints rg's live stats block (and any matches) to stdout, silently breaking `--quiet`'s "no incidental output" promise that the slow path enforces explicitly (`and not quiet` at the `is_empty` note — `grep -n "and not quiet" src/tensor_grep/cli/main.py`, was `:8468`, now `:8608`; the `if quiet: sys.exit(...)` gate — `grep -n "if quiet:" src/tensor_grep/cli/main.py`, was `:8478-8480`, now `:8618-8620`). Untested (`--stats --quiet` appears nowhere under `tests/`) and **not platform-specific** — it reproduces wherever `rg` is installed, including ordinary user machines.
...
Before #702, the plain flag-less `bootstrap._run_rg_passthrough` path (`grep -n "^def _run_rg_passthrough" src/tensor_grep/cli/bootstrap.py` — was `:1088`, now `:1337` — the front
...
- **Evidence labeling** — routing/claim evidence is labeled `parser-backed | rg-backed | graph-derived | heuristic | LSP-confirmed | stale/uncertain`; when signals disagree, downgrade confidence and surface the contradiction rather than hiding it behind one ranked file (`docs/CONTRACTS.md`, `grep -n "parser-backed"` — was `:109`, now `:152`). LSP availability is **not** semantic proof: a row counts as `lsp_proof` only with an explicit `lsp_provider_response = true` (`docs/CONTRACTS.md`, `grep -n "lsp_provider_response = true"` — was `:110`, now `:153`).
...
(`_select_ast_backend_for_pattern` — `grep -n "^def _select_ast_backend_for_pattern" src/tensor_grep/cli/main.py`, was `:6737`, now `:6915` -- cite by symbol; the older `:6690-6707` range now holds unrelated #299 code), not a hardware gate. **Correction (this pass, 2026-08-01): the cpu_backend.py ReDoS-gate cites (§7 below) were NOT byte-stable as the prior pass claimed** — `_fallback_pattern_is_provably_linear` (`:280`->`:316`), `_RustUtf8DecodeMismatch` (`:438`->`:60`), and `_search_ltl` (`:931`->`:983`) had all drifted; only `_search_word_line_context_via_rust` (`:852`) held. The ripgrep_backend.py `result_incomplete`-setting lines (§ Partial-results contract) stayed close (`:144/325/444` vs the cited `:145/326/445`), but its rg-missing guard drifted (`:505`->`:541`). **The lesson is the one this section already states: a "found unchanged" claim about a growing file is itself a claim with a short half-life — it was wrong four sessions after being written.** Re-verify anything volatile before relying on it:

- **Version:** `grep '^version' pyproject.toml` (was `1.95.0`).
- **Front-door entry point:** `grep -n "^def main_entry" src/tensor_grep/cli/bootstrap.py` (this line has now carried THREE different stale numbers across passes -- `:1398` as of 2026-07-27, `:1444` elsewhere in this very file, actual today `:1466` -- never trust any of them, always re-grep) and confirm `pyproject.toml`/`packaging` still points `tg` at `tensor_grep.cli.bootstrap:main_entry`.
...
- **Flag front doors (2):** `rust_core/src/main.rs:204` (`SEARCH_PYTHON_PASSTHROUGH_FLAGS`, unchanged), `bootstrap.py:50` (`_TG_ONLY_SEARCH_FLAGS`, moved from `:38`, unchanged since).
- **Native-delegation cites (3):** `grep -n "^def _can_delegate_to_native_tg_search\|^def _build_native_tg_search_command\|_NATIVE_TG_DELEGATION_DEFAULT_REQUIRED_FIELDS = " src/tensor_grep/cli/main.py` -- this exact bullet previously cited `:3709` for BOTH `_can_delegate_to_native_tg_search` AND `_build_native_tg_search_command` (one line number for two different functions, itself a symptom of hand-copied numbers rather than a fresh grep); actual today: `_can_delegate_to_native_tg_search` `:3794`, `_build_native_tg_search_command` `:3816`, `_NATIVE_TG_DELEGATION_DEFAULT_REQUIRED_FIELDS` `:1894` (this last one was already correct).
...
- **rg-passthrough `--` gap: RESOLVED, not open.** `grep -n "^fn ripgrep_operand_args" rust_core/src/rg_passthrough.rs` (`:581-600` cited previously, def now at `:584`, close enough not to be the drift story here) inserts `--` before paths; its 3 tests -- `grep -n "^    fn operand_args_" rust_core/src/rg_passthrough.rs` -- were cited at `:787-824`, now at `:1034-1071` (roughly 250 lines adrift). Do not re-open weak-point §5 as unresolved.
- **Registration CI gate BLOCKING:** confirm in `AGENTS.md` §"Adding a Command or Flag" (starts at line 396 as of this writing; as of #282/v1.17.1 for when it went blocking, itself now stated at `AGENTS.md` -- `grep -n "BLOCKING since v1.17.1"`, was `:414`, now `:516`) — grep the header text, the line number shifts as sections are added above it.
- **Partial-results contract (added 2026-07-03, #341/`f11ce28`; exit-code side made stricter by #398/#399/#401):** `grep -n "result_incomplete: bool\|incomplete_reason: str" src/tensor_grep/core/result.py` (`SearchResult.result_incomplete`/`incomplete_reason`; cited elsewhere in this file as both `:54-55` and `:67+` -- both wrong, actual `:111-112`) + `grep -n "^def merge_runtime_routing" src/tensor_grep/core/result.py` (`merge_runtime_routing` OR-merge; cited elsewhere as both `:135+` and `:67+` -- both wrong, actual `:142`); exit-code wiring `grep -n "result_incomplete else\|exit_incomplete else" src/tensor_grep/cli/main.py` (was a contiguous `:8175-8240`, now scattered `:8549-8620`; exit 2 fires regardless of found, per #401 — `docs/CONTRACTS.md`, `grep -n "mirrors .tg search.'s .2 = result_incomplete."` — was `:114`, now `:157`, the symbol-command exit-code contract); envelope `grep -n "result_incomplete" src/tensor_grep/cli/formatters/json_fmt.py` (was `:126-127,189-190`, the first pair is still current, the second has drifted); MCP `grep -n '"result_incomplete"' src/tensor_grep/cli/mcp_server.py` (was 5 representative sites at `:2161,4693,4866,4960,5278`, ALL five have since drifted — this file grows fast, re-grep rather than cite a fixed list). Re-verify: `tests/unit/test_rg_exit2_partial.py` green.
...
- **B9 (ReDoS gate, RESOLVED audit #6/#16/#111):** `grep -n "_fallback_pattern_is_provably_linear\|needs_word_or_context_rust_routing" src/tensor_grep/backends/cpu_backend.py` should find the fixed_strings-only gate (was `:280`, now `:316`) plus its call sites in BOTH the `_RustUtf8DecodeMismatch` handler AND the generic native-failure `except Exception` handler; the gate must admit ONLY `fixed_strings` (a static pattern-char allow-list was proven unsound by the Opus gate — the `(a|aa)...b` alternation bomb). **Do not trust a "byte-stable"/"independently re-confirmed" claim about this cluster — a prior pass asserted `:280`/`:438`/`:485`/`:512`/`:800`/`:931` were all byte-stable at v1.95.0, and by this pass three of those (`:280`, `:438`, `:931`) had already drifted (see the correction near the top of the Provenance section) and `:800`/`:485`/`:512` do not cleanly resolve to a single call site at all — always re-grep this cluster fresh, never cite the last pass's numbers.** Re-check before citing closed — a future new fallback path could reopen the class the way #111 did after #6/#16.
- **Native-delegation forward-or-refuse contract (added 2026-07-03, #342/`5e6f780`):** gate `grep -n "^def _can_delegate_to_native_tg_search" src/tensor_grep/cli/main.py` (was `:3709`, now `:3794`), forward builder `grep -n "^def _build_native_tg_search_command" src/tensor_grep/cli/main.py` (was `:3731`, now `:3816` — note this line was previously ALSO mis-cited as `:3709`, the same number as the gate function, two paragraphs earlier in this same file), refuse-tuple `grep -n "_NATIVE_TG_DELEGATION_DEFAULT_REQUIRED_FIELDS = " src/tensor_grep/cli/main.py` (`:1894`, unchanged this pass). Re-verify: run `tests/unit/test_native_delegation_field_coverage.py` after touching `SearchConfig` — a new unclassified field turns it red immediately, which is the point.
- **AST-routing GPU-vs-DSL correction (fact fixed 2026-07-24, tracing v1.64.4/#542):** `AstBackend.is_available()` — `grep -n "def is_available" src/tensor_grep/backends/ast_backend.py` (was `:505`, still `:505` this pass; docstring through was `:512` now `:513`; `return` was `:517` now `:518` — explicitly disclaims torch/CUDA); wrapper-preference policy — `grep -n "ast_wrapper.is_available()" src/tensor_grep/cli/main.py` (`if ast_wrapper.is_available(): ... elif pattern_kind == "native": if ast_backend.is_available(): ...`; this bullet previously cited `:6690-6707`, which is the SAME stale range already flagged as wrong earlier in this file (see "Native-vs-Python routing" above) — actual call site is now `:6958-6960`, inside `_select_ast_backend_for_pattern` at `:6915`). Re-verify: `python -c "from tensor_grep.backends.ast_backend import AstBackend; import inspect; print(inspect.getsource(AstBackend.is_available))"` should show no `torch`/`cuda` reference.
- **`MatchLine` hashability (added 2026-07-03, #344/`80de0b4`):** `grep -n "^class MatchLine" src/tensor_grep/core/result.py` (was `:4-17`, now `:55`; `:4` is a different symbol, `strip_line_terminator`) (`submatches` field uses `compare=False`). Re-verify: `python -c "from tensor_grep.core.result import MatchLine; hash(MatchLine(1,'x','f.py',submatches=({'start':0,'end':1},)))"` must not raise.
