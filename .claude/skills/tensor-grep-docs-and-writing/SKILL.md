...
2. **`.claude/skills/tensor-grep/SKILL.md`** — the **tg-usage skill**: command patterns, argument order, the registration-audit workflow, for an agent *driving* `tg`. Same basename, unrelated content and governance. It carries its own release note ("Current State", `release_docs_current_tag:`) that mirrors — but is not identical to — the root doc's prose; **do not assume the two `SKILL.md` files need the same edit.** Exactly one pytest reads it: `tests/unit/test_benchmark_scripts.py::test_tensor_grep_claude_skill_should_require_non_interactive_action`, which asserts the Non-Interactive Mode section still contains `"do not ask for confirmation"` and `"make the change directly"` (`.claude/skills/tensor-grep/SKILL.md:73-75`). AGENTS.md's own Skills section (`AGENTS.md:249`) says to "Keep it in sync whenever commands/flags change" — that sync is currently **discipline, not full pytest coverage**; only those two literal sentences are machine-checked.
