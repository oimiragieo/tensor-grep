...
3. **`.claude/skills/<topic>/SKILL.md`** (this file's siblings — `tensor-grep-change-control`, `tensor-grep-architecture-contract`, etc.) — narrow runbooks, one per topic. As of 2026-07-29, **five test files reference this library** (`grep -rln '\.claude/skills' tests/`): `test_skill_index_sync.py`, `test_pyproject_dependencies.py`, `test_orient_deweight_vendored.py`, `tests/eval/test_agent_accuracy.py`, `test_benchmark_scripts.py`. The claim here used to be "none are pytest-pinned, only `test_benchmark_scripts.py` matches" -- already false when it was last "re-verified" on 2026-07-23, because `test_skill_index_sync.py` landed 2026-07-14. Its own hedge ("a future governance test could start pinning this library") had already come true. **What is still true**: no test pins the PROSE of an individual topic SKILL.md; `test_skill_index_sync.py` pins the folder SET against the AGENTS.md/CLAUDE.md indices, not the content. Re-run the grep -- and if the count has moved again, this line is stale, not the tests.
