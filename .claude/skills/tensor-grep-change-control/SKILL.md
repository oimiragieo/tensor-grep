...
**Domain note (ripgrep):** tg's default regex path matches invalid UTF-8; **PCRE2 requires valid UTF-8 and transcodes** — which is *why* swapping `--pcre2` to a non-PCRE2 engine changes results, not just performance. `rg` exit code `2` is a **non-fatal** error (not necessarily fatal); exit `1` with empty output is a legitimate "no match" (`AGENTS.md:155`).
