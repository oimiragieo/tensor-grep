...
| **Evidence** | The C# PR (`6c09424`, #726) explicitly self-corrects this exact framing in its own commit message: "mirroring the Go precedent (`lang_go.py`) rather than the older pre-registry Rust/JS/TS inline pattern -- Go is the most current template since it was added after the `lang_registry` refactor." The PHP PR (`8659e87`, #724) independently states the same: "mirroring `lang_go.py`'s module shape." Both `lang_php.py` and `lang_csharp.py` exist as standalone modules on `origin/main`; there is no `lang_java.py` — Java (`f3ad51b`, #725) legitimately used the OTHER valid shape instead, with its extractor functions inline in `repo_map.py`, self-documented there as "mirroring the two Rust functions above" (`repo_map.py:4515-4519`). Both shapes register through `lang_registry` and are contract-consistent — see `_target_language_for_path` (`repo_map.py:7850-7880`), which every one of the four newly-registered suffixes (go/java/php/csharp) touches with its own inline "MOST-FORGOTTEN seam" comment reminding the next language's author why the branch matters. |
