...
- Main CI run `25762981815` passed the pre-release matrix, semantic-release, PyPI artifact validation, `publish-github-release-assets`, `publish-pypi`, and `publish-success-gate`. CodeQL run `25762981305` passed on the `v1.10.6` release line.
- PyPI pinned public install resolves with `uvx --refresh-package tensor-grep --from tensor-grep==1.10.6 tg --version`
...
- Public `v1.10.6` dogfood verified `tg upgrade`, direct managed native `tg.exe`, fresh `cmd /c tg --version`, fresh `pwsh -NoProfile -Command "tg --version"`, PyPI, and GitHub assets. Python `subprocess.run(["tg", ...])` remains a separate readiness route because Windows `CreateProcess` can choose a foreign same-directory `.exe` even when shells prefer a tensor-grep `.com` bridge.
...
- GPU support exists and local devices can be detected, but GPU routing is benchmark-governed. Public `v1.10.6` managed GPU requests still route through `GpuSidecar` and are unsupported for promotion; only qualifying public `NativeGpuBackend` rows with `sidecar_used = false`, correctness, and wins over both `rg` and `tg_cpu` can support GPU marketing. Local CUDA-feature `v1.10.6` follow-up work shows a real many-pattern lane (`100` fixed strings over 1GB beat sequential `rg` end-to-end), but single-pattern cold grep still loses. The near-term public performance story remains large-file CPU search until CUDA-native public assets prove the same lane.
