...
- Tagged merge commit: `21627d2 fix: harden v1.12.8 dogfood contracts`
- Tagged release commit: `b15f71a chore(release): v1.12.9 [skip ci]`
...
- Main CI run `25941933937` passed the pre-release matrix, semantic-release, PyPI artifact validation, `publish-github-release-assets`, `publish-pypi`, and `publish-success-gate`. CodeQL run `25941933444` passed on the `v1.12.9` release line.
- PyPI pinned public install resolves with `uvx --refresh-package tensor-grep --from tensor-grep==1.12.9 tg --version`
...
- Public `v1.12.9` dogfood verified PyPI, `uvx`, GitHub assets, public launcher resolution, bounded map/context output, `tg run --pattern`, root cold rg-shaped routing, accepted search/native-front-door CLI drift fixes, `tg new --base-dir`, edit-plan budget flags, explicit rg JSON Lines routing via `--format rg --json`, and explicit JSON/NDJSON schema positioning; public managed GPU remains not promotion-ready and falls back to CPU or unsupported rows unless a CUDA-feature native build proves `NativeGpuBackend` with `sidecar_used = false`.
