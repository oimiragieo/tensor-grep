...
- Main CI run `25774742206` passed the pre-release matrix, semantic-release, PyPI artifact validation, `publish-github-release-assets`, `publish-pypi`, and `publish-success-gate`. CodeQL run `25775170033` passed on the `v1.10.7` release line.
- PyPI pinned public install resolves with `uvx --refresh-package tensor-grep --from tensor-grep==1.10.7 tg --version`
...
- Public `v1.10.7` dogfood verified `tg upgrade`, direct managed native `tg.exe`, fresh `cmd /c tg --version`, fresh `pwsh -NoProfile -Command "tg --version"`, PyPI, and GitHub assets. Python `subprocess.run(["tg", ...])` remains a separate readiness route because Windows `CreateProcess` can choose a foreign same-directory `.exe` even when shells prefer a tensor-grep `.com` bridge.
