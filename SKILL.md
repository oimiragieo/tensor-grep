...
- Public `v1.10.10` dogfood verified the pinned managed installer, direct managed native `tg.exe`, fresh `cmd /c tg --version`, fresh `pwsh -NoProfile -Command "tg --version"`, Python `subprocess.run(["tg", ...])`, PyPI, `uvx`, and GitHub assets after `tg repair-launcher --allow-foreign-rename` backed up the operator-owned foreign Together CLI `tg.exe` and installed the verified managed native front door into that PATH slot.
