...
- PR #90 `fix: harden v1.9.9 dogfood followups` merged and released as `v1.9.10` on GitHub assets; PyPI publication is pending the release-wheel retry follow-up
...
- Main CI run `25645819170` passed the pre-release matrix, semantic-release, and `publish-github-release-assets`; the PyPI wheel matrix hit a transient crates.io DNS failure and `publish-success-gate` failed as designed. CodeQL run `25646156907` passed on the `v1.9.10` release commit.
