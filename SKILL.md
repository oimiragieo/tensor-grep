...
- PR #72 `fix: expose launcher route observability` merged and released
...
- Main CI run `25581373995` passed through semantic-release, PyPI artifact validation, `publish-github-release-assets`, `publish-pypi`, and `publish-success-gate`; CodeQL/dynamic run `25581373725` passed on the release-bearing merge commit and release-commit dynamic run `25581894666` passed
- PyPI latest and pinned public install both resolve `tensor-grep==1.8.32`
...
- Post-release fast gate before the `v1.8.32` PR: `python scripts/agent_readiness.py --output artifacts/agent_readiness_launcher_observability.json` passed all checks.
