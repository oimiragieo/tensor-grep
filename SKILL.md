...
- Main CI run `25836697091` passed the pre-release matrix, semantic-release, PyPI artifact validation, `publish-github-release-assets`, `publish-pypi`, and `publish-success-gate`. CodeQL run `25836696835` passed on the `v1.11.1` release line.
- PyPI pinned public install resolves with `uvx --refresh-package tensor-grep --from tensor-grep==1.11.1 tg --version`
- GitHub release assets for `v1.11.1` include native CPU front doors, checksums, winget manifest, Homebrew formula, and publish instructions
- Public `v1.11.1` dogfood verified PyPI, `uvx`, GitHub assets, and the agent capsule hardcase release proof; `v1.10.10` remains the historical Windows subprocess launcher repair release.
